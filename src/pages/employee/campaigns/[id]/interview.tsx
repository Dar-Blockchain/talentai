...
import {
  ...
  Snackbar, Alert, Container, Button, CircularProgress, IconButton,
} from '@mui/material';
...
const CampaignHeader: React.FC<HeaderProps> = ({
  campaign, moduleType, interviewStatus, isVoiceActive,
  cameraStatus, agentState, coverage, onBack, onEnd,
}) => {
  ...
  return (
    <Box sx={{ bgcolor: '#fff', borderBottom: '1px solid #E5E7EB', px: { xs: 2, md: 4 }, py: 0, position: 'sticky', top: 0, zIndex: 100, boxShadow: '0 1px 3px rgba(0,0,0,0.06)' }}>
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', py: 1.75, gap: 2, flexWrap: 'wrap' }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
          <IconButton size="small" onClick={onBack} disabled={isActive} sx={{ color: '#6B7280', '&:hover': { color: '#111827' } }}>
          ...
          <Box>
            <Typography sx={{ fontSize: 15, fontWeight: 700, color: '#111827', lineHeight: 1.2 }}>{campaign.title}</Typography>
            <Typography sx={{ fontSize: 11, color: '#6B7280' }}>
              {meta.label}{moduleType === 'SKILL_TEST' && (campaign.module as any)?.config?.skill ? ` · ${(campaign.module as any).config.skill}` : ''}
            </Typography>
          </Box>
        </Box>
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, flexWrap: 'wrap' }}>
          <Chip
            label={interviewStatus === 'idle' ? 'Ready' : interviewStatus === 'connecting' ? 'Connecting…' : isActive ? '● Live' : interviewStatus === 'ended' ? 'Completed' : interviewStatus}
            size="small"
            sx={{ fontWeight: 700, fontSize: 11, height: 22, bgcolor: isActive ? '#ECFDF5' : '#F3F4F6', color: isActive ? '#059669' : '#6B7280', border: isActive ? '1px solid #A7F3D0' : '1px solid #E5E7EB' }}
          />
          {isVoiceActive && <Chip size="small" label="🎤 Speaking" sx={{ bgcolor: '#EFF6FF', color: '#2563EB', fontSize: 11, height: 22 }} />}
          {agentState === 'thinking' && <Chip size="small" label="🧠 AI Thinking" sx={{ bgcolor: '#FFFBEB', color: '#D97706', fontSize: 11, height: 22 }} />}
          {isActive && coverage && (
            <Chip
          ...
          {isActive && (
            <Button
              ...
              onClick={onEnd}
              sx={{ bgcolor: '#FEF2F2', color: '#EF4444', border: '1px solid rgba(239,68,68,0.2)', fontWeight: 700, fontSize: 12, textTransform: 'none', borderRadius: 2, px: 2, py: 0.6, '&:hover': { bgcolor: '#FEE2E2' } }}
            >
              End
            </Button>
          )}
...
const StatusPanel: React.FC<{
  interviewStatus: string; moduleType: string; isHydrated: boolean;
  connectionStatus: string; cameraStatus: string;
  onStart: () => void; onViewResults: () => void;
  ...
  if (interviewStatus === 'connecting') {
    return (
      <Box sx={{ textAlign: 'center', py: 10 }}>
        ...
        <Typography sx={{ fontSize: 18, fontWeight: 600, color: '#111827' }}>Initializing Assessment…</Typography>
        <Typography sx={{ fontSize: 13, color: '#6B7280', mt: 1 }}>Setting up your AI session</Typography>
      </Box>
    );
  }
  ...
  if (interviewStatus === 'ended') {
    return (
      <Box sx={{ textAlign: 'center', py: 8, bgcolor: '#ECFDF5', borderRadius: 3, border: '1.5px solid #A7F3D0' }}>
        ...
        <Typography sx={{ fontSize: 22, fontWeight: 800, color: '#111827', mb: 1 }}>Assessment Complete!</Typography>
        ...
        <Button
          ...
          sx={{ background: 'linear-gradient(135deg, #10B981 0%, #059669 100%)', boxShadow: '0 4px 14px rgba(16,185,129,0.35)', fontWeight: 700, fontSize: 14, textTransform: 'none', borderRadius: 2.5, px: 4, py: 1.4, '&:hover': { boxShadow: '0 6px 20px rgba(16,185,129,0.45)', transform: 'translateY(-1px)' }, transition: 'all 0.25s' }}
        >
          View Results
        </Button>
      </Box>
  ...
  return (
    <Box sx={{ textAlign: 'center', py: 7, px: 3, background: 'linear-gradient(135deg, rgba(102,126,234,0.04) 0%, rgba(118,75,162,0.04) 100%)', borderRadius: 3, border: '2px dashed rgba(102,126,234,0.2)' }}>
      <Box sx={{ width: 72, height: 72, borderRadius: 3, background: meta.gradient, display: 'flex', alignItems: 'center', justifyContent: 'center', mx: 'auto', mb: 3, boxShadow: `0 8px 24px ${meta.shadow}` }}>
        <ModIcon sx={{ fontSize: 36, color: '#fff' }} />
      </Box>
      <Typography sx={{ fontSize: 22, fontWeight: 800, color: '#111827', mb: 1 }}>Ready to Begin?</Typography>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'center', gap: 1.5, mb: 4, flexWrap: 'wrap' }}>
        {[{ label: 'Connection', ok: connectionStatus === 'connected' }, { label: 'Camera', ok: cameraStatus === 'granted' }, { label: 'Microphone', ok: isHydrated }].map(({ label, ok }) => (
          <Box key={label} sx={{ display: 'flex', alignItems: 'center', gap: 0.75, px: 1.5, py: 0.6, borderRadius: 5, bgcolor: ok ? '#F0FDF4' : '#FEF2F2', border: `1px solid ${ok ? '#BBF7D0' : '#FECACA'}` }}>
            <Box sx={{ width: 7, height: 7, borderRadius: '50%', bgcolor: ok ? '#22C55E' : '#EF4444' }} />
            <Typography sx={{ fontSize: 12, fontWeight: 600, color: ok ? '#16A34A' : '#DC2626' }}>{label}</Typography>
          </Box>
        ))}
      </Box>
      <Button
        ...
        sx={{ background: canStart ? meta.gradient : undefined, boxShadow: canStart ? `0 4px 18px ${meta.shadow}` : 'none', fontWeight: 700, fontSize: 15, textTransform: 'none', borderRadius: 2.5, px: 5, py: 1.5, '&:hover': { boxShadow: `0 8px 24px ${meta.shadow}`, transform: 'translateY(-1px)' }, '&:disabled': { bgcolor: '#D1D5DB', color: '#9CA3AF' }, transition: 'all 0.25s' }}
      >
        Start Assessment
      </Button>
      {cameraStatus !== 'granted' && cameraStatus !== 'requesting' && (
        <Typography sx={{ mt: 2, fontSize: 12, fontWeight: 500, color: '#F59E0B' }}>⚠️ Camera access is required to start</Typography>
      )}
    </Box>
  );
};
...
const UnsupportedModule: React.FC<{ moduleType: string; onBack: () => void }> = ({ moduleType, onBack }) => (
  <Box sx={{ textAlign: 'center', py: 10 }}>
    ...
    <Typography sx={{ fontSize: 20, fontWeight: 700, color: '#111827', mb: 1 }}>Module not supported here</Typography>
    <Typography sx={{ fontSize: 14, color: '#6B7280', mb: 4 }}>The <strong>{moduleType}</strong> module cannot be taken through this interface.</Typography>
    <Button variant="outlined" startIcon={<ArrowBackOutlined />} onClick={onBack} sx={{ textTransform: 'none', borderRadius: 2, fontWeight: 600 }}>
      Back to Campaign
    </Button>
  </Box>
);
...
const EmployeeCampaignInterview: React.FC = () => {
  ...
  const socket = useInterviewSocket({
    ...
    onReportUpdate:     useCallback((r: RealTimeReport)          => setRealTimeReport(r),                      []),
  ...
  handlersRef.current = {
    ...
    onVoiceActivity: (data) => { audio.setIsVoiceActive(data.isActive); if (data.isActive) audio.setLastVoiceActivity(Date.now()); },
    onInterviewEnded: (data) => {
      ...
      if (data.finalReport || data.analytics) {
        localStorage.setItem('last_interview_analysis', JSON.stringify({ finalReport: data.finalReport, analytics: data.analytics, sessionId: data.sessionId, interviewType: interviewConfig?.interviewType || 'CAMPAIGN', campaignId: id, participantId, timestamp: new Date().toISOString() }));
      }
    },
    onInterviewError: () => { audio.setAgentState('waiting'); audio.setAgentMessage('Something went wrong. You can re-submit your answer or continue.'); },
  };
  ...
  const startInterview = useCallback(async () => {
    if (!socket.socketRef.current || !socket.isConnected) { showNotification('Not connected to interview system', 'error'); return; }
    try {
      socket.setInterviewStatus('connecting');
      await audio.initializeAudio();
      socket.socketRef.current.emit('start_interview', {
        config: { ...interviewConfig, campaignId: id, participantId, moduleType, silenceIntelligence: { interviewType: interviewConfig.interviewType, candidateBehavior: { interactionStyle: 'balanced', confidenceLevel: 'medium', communicationStyle: 'mixed' }, adaptiveMode: true, contextualAdjustments: true } },
  ...
  // Back → campaign details page (not the list)
  ...
  return (
    <>
      ...
      <Box sx={{ minHeight: '100vh', bgcolor: '#F8F9FA', display: 'flex', flexDirection: 'column' }}>
        <CampaignHeader campaign={campaign} moduleType={moduleType} interviewStatus={socket.interviewStatus} isVoiceActive={audio.isVoiceActive} cameraStatus={camera.cameraStatus} agentState={audio.agentState} coverage={coverage} onBack={handleBack} onEnd={endInterview} />
            ...
            <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: '7fr 3fr' }, gap: 3 }}>
              <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2.5 }}>
                {isActive && lastInterviewerMessage && (
                  <QuestionPanel currentMessage={lastInterviewerMessage} isInReadingTime={audio.isInReadingTime} readingTimeLeft={audio.readingTimeLeft} questionHighlight={audio.questionHighlight} />
                )}
                <CameraPreview videoRef={camera.videoRef} cameraStatus={camera.cameraStatus} cameraError={camera.cameraError} isConnecting={audio.isConnecting} interviewStatus={socket.interviewStatus} audioContextRef={audio.audioContextRef} attachStream={camera.attachStream} />
                <CoverageDashboard interviewStatus={socket.interviewStatus} coverage={coverage} realTimeReport={realTimeReport} agentMessage={audio.agentMessage} coverageDashboardExpanded={audio.coverageDashboardExpanded} onToggleExpand={() => audio.setCoverageDashboardExpanded(!audio.coverageDashboardExpanded)} />
              </Box>

              <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2.5 }}>
                {socket.interviewStatus !== 'active' && (
                  <Box sx={{ bgcolor: '#fff', borderRadius: 3, border: '1px solid #E5E7EB', p: 3 }}>
                    <StatusPanel interviewStatus={socket.interviewStatus} moduleType={moduleType} isHydrated={socket.isHydrated} connectionStatus={socket.connectionStatus} cameraStatus={camera.cameraStatus} onStart={startInterview} onViewResults={handleViewResults} />
                  </Box>
                )}
                {isActive && (
                  <AgentStatusPanel interviewStatus={socket.interviewStatus} agentState={audio.agentState} agentMessage={audio.agentMessage} isInReadingTime={audio.isInReadingTime} readingTimeLeft={audio.readingTimeLeft} accumulatedTurns={audio.accumulatedTurns} isVoiceActive={audio.isVoiceActive} currentTranscript={audio.currentTranscript} debugMode={audio.debugMode} setDebugMode={audio.setDebugMode} silenceDebugLog={audio.silenceDebugLog} transcriptDebugLog={audio.transcriptDebugLog} onSubmitAnswer={audio.sendAccumulatedAnswer} />
                )}
              </Box>
            </Box>
          </Container>
        ...
        <GDPRConsentModal open={!camera.consentGiven} onAccept={camera.giveConsent} onDecline={handleBack} />
        ...
        <SecurityModals showFirstViolationModal={security.showFirstViolationModal} showSecurityModal={security.showSecurityModal} onDismissFirst={() => security.setShowFirstViolationModal(false)} onDismissSecond={() => security.setShowSecurityModal(false)} onReturnToDashboard={handleBack} />
