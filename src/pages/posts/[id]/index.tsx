...
const PostDetails: React.FC = () => {
  ...
  const [activeEdit, setActiveEdit] = React.useState<"post" | "recruitment" | "agent" | null>(null);
  ...
  return (
    <Box sx={{ minHeight: "100vh", backgroundColor: "rgba(251, 254, 255, 1)", py: 2 }}>
      <Container maxWidth="lg">
        <HeaderDashboard />

        <Button
          startIcon={<ArrowBack sx={{ color: "#10b981", transition: "transform 0.2s easeIn" }} />}
        ...
        {loading && <Loader title="Loading job details…" subtitle="Please wait while we load the job information." />}
        {!loading && error && <Alert severity="error" sx={{ mt: 3 }}>{error || "Failed to load job details."}</Alert>}
        ...
        {!loading && !error && job && (
          ...
          >
            {activeEdit === "post" && <EditPostDetails onCancel={handleCancelEdit} />}
            {activeEdit === "recruitment" && <EditRecruitmentFlow onCancel={handleCancelEdit} />}
            {activeEdit === "agent" && <EditAgentConfiguration onCancel={handleCancelEdit} />}

            {activeEdit === null && (
              <>
                <PostBasicDetails onEdit={() => setActiveEdit("post")} />
                <RecruitmentFlowDetails onEdit={() => setActiveEdit("recruitment")} />
                <AgentConfigurationDetails onEdit={() => setActiveEdit("agent")} />
              </>
            )}
          </Box>
        )}
