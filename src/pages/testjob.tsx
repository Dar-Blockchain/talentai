...
export default function TestJob() {
  ...
  // Extract skills and levels from URL parameters
  useEffect(() => {
    const { skills, levels } = router.query;
    ...
    const levelsArray = typeof levels === 'string' ? levels.split(',') : Array.isArray(levels) ? levels : [];

    if (skillsArray.length === 0 || levelsArray.length === 0 || skillsArray.length !== levelsArray.length) {
    ...
    const fetchQuestions = async () => {
      try {
        ...
        for (let i = 0; i < skillsArray.length; i++) {
          ...
          const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}evaluation/generate-technique-questions`, {
            method: 'POST',
            headers: {
            ...
            body: JSON.stringify({
              ...
              proficiencyLevel: parseInt(level),
            }),
          });

          if (!response.ok) {
            console.error(`Failed to fetch questions for ${skill}`);
            continue;
          }

          const data = await response.json();
          const questionsWithMetadata = data.questions.map((text: string) => ({
            id: uuidv4(),
            text,
            skill,
            level,
