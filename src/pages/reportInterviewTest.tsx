  ...
  useEffect(() => {
    ...
    const analyzeResults = async () => {
      try {
        ...
        const testInterviewToken = localStorage.getItem('testInterview') || Cookies.get('testInterview');
        ...
        // Check if this is an HR assessment
        if (testData.metadata?.type === 'hr_assessment') {
          console.log('Debug tokens:', {
            apiToken: token ? 'exists' : 'missing',
            testInterviewToken: testInterviewToken ? 'exists' : 'missing',
            localStorageTestInterview: localStorage.getItem('testInterview'),
            cookiesTestInterview: Cookies.get('testInterview')
          });

          // Use testInterview token if available, otherwise fall back to api_token
          const authToken = testInterviewToken || token;

          if (!authToken) {
            throw new Error('No authentication token found for HR assessment');
          }

          console.log('Using auth token:', authToken ? 'exists' : 'missing');
          ...
          if (Array.isArray(testData.results) && testData.results.length > 0) {
            // Check if results is an array of objects with question/answer properties
            if (testData.results[0].hasOwnProperty('question') && testData.results[0].hasOwnProperty('answer')) {
              questions = testData.results.map((r: any) => r.question);
              answers = testData.results.map((r: any) => r.answer);
            } else {
          ...
          if (!hrResponse.ok) {
            ...
            let errorMessage = 'Failed to analyze HR assessment results';
            try {
              const errorJson = JSON.parse(errorText);
              errorMessage = errorJson.message || errorJson.error || errorText;
            } catch {
              errorMessage = errorText || 'Failed to analyze HR assessment results';
            }
            throw new Error(`Failed to analyze HR assessment results: ${errorMessage}`);
          }

          const analysisData = await hrResponse.json();
          console.log('HR Analysis response:', analysisData);
