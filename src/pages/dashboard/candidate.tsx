            ...
            <Box sx={{ display: { xs: "none", md: "flex" }, flexDirection: "column", gap: 2 }}>
              ...
              <Box sx={{ bgcolor: "#fff", borderRadius: "16px", border: "1px solid #E5E7EB", overflow: "hidden", boxShadow: "0 1px 4px rgba(0,0,0,0.04)" }}>
                ...
                <Box sx={{ px: 2, pb: 2 }}>
                  ...
                  {user?.email && <Typography sx={{ fontSize: "0.72rem", color: "#9CA3AF", mt: 0.25, mb: 1 }}>{user.email}</Typography>}
                  {profile?.targetRole && (
                    <Chip label={profile.targetRole} size="small" sx={{ fontSize: "0.65rem", height: 20, bgcolor: TBG, border: `1px solid ${TBRD}`, color: T, fontWeight: 600, mb: 1 }} />
                  )}
