...
import { ADMIN_CHART_COLORS } from '@/modules/admin/shared';
...
const PlanOverviewGrid: React.FC<PlanOverviewGridProps> = ({ plans }) => {
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 mb-6">
      {plans.map((plan, i) => {
        const color = ADMIN_CHART_COLORS[i % ADMIN_CHART_COLORS.length];
        return (
          <Card key={plan._id} className="shadow-none transition-all hover:-translate-y-0.5 hover:shadow-md">
            <div className="p-4">
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  <div className="w-8 h-8 rounded-lg flex items-center justify-center shrink-0" style={{ background: `${color}14` }}>
                    <WorkspacePremiumIcon style={{ fontSize: 16, color }} />
                  </div>
