...
import { SectionTitle, FieldLabel } from "@/modules/settings/shared/components";
import { TEAL, TEAL_BG, TEAL_BORDER, AVAILABLE_SCOPES, fieldSx, fmtDate } from "@/modules/settings/shared/constants";
...
const ApiKeysTab: React.FC = () => {
  ...
  useEffect(() => {
    if (editingKey) {
      ...
      setEditForm({
        name: editingKey.name, serviceName: editingKey.serviceName, scopes: editingKey.scopes,
  ...
  const handleCreate = async () => {
    try {
      ...
      setCreateOpen(false); setForm(DEFAULT_FORM);
  ...
  const ipToggleSx = { mb: 1, "& .MuiToggleButton-root": { textTransform: "none", fontFamily: "Poppins", fontSize: "0.78rem", fontWeight: 600, px: 2, borderRadius: "8px !important", "&.Mui-selected": { bgcolor: TEAL, color: "#fff", "&:hover": { bgcolor: "#0F766E" } } } };
  ...
  return (
    <Box sx={{ p: { xs: 2.5, md: 3.5 } }}>
            ...
            <Box key={k.id} sx={{ border: "1px solid #E5E7EB", borderRadius: "12px", p: 2, display: "flex", alignItems: "center", gap: 2, flexWrap: "wrap" }}>
              ...
              <Box sx={{ flex: 1, minWidth: 0 }}>
                <Box sx={{ display: "flex", alignItems: "center", gap: 1, flexWrap: "wrap" }}>
                  ...
                  <Chip label={k.isActive ? t("pages.settings.api_keys.active") : t("pages.settings.api_keys.disabled_label")} size="small"
      ...
      <Dialog open={createOpen} onClose={() => setCreateOpen(false)} maxWidth="sm" fullWidth PaperProps={{ sx: { borderRadius: "16px" } }}>
        ...
        <Divider />
        <DialogForm fields={<>
          <Box><FieldLabel text={t("pages.settings.api_keys.fields.key_name")} /><TextField value={form.name} onChange={(e) => setForm((f) => ({ ...f, name: e.target.value }))} fullWidth placeholder={t("pages.settings.api_keys.fields.key_name_placeholder")} sx={fieldSx} /></Box>
          <Box><FieldLabel text={t("pages.settings.api_keys.fields.service_name")} /><TextField value={form.serviceName} onChange={(e) => setForm((f) => ({ ...f, serviceName: e.target.value }))} fullWidth placeholder={t("pages.settings.api_keys.fields.service_placeholder")} sx={fieldSx} /></Box>
          <Box><FieldLabel text={t("pages.settings.api_keys.fields.scopes")} /><ScopeChips /></Box>
          <Box sx={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 2 }}>
            <Box><FieldLabel text={t("pages.settings.api_keys.fields.rate_limit")} /><TextField type="number" value={form.rateLimit} onChange={(e) => setForm((f) => ({ ...f, rateLimit: Number(e.target.value) }))} fullWidth inputProps={{ min: 1 }} sx={fieldSx} /></Box>
          ...
          <Box>
            ...
            <ToggleButtonGroup exclusive size="small" value={form.ipMode} onChange={(_, v) => v && setForm((f) => ({ ...f, ipMode: v }))} sx={ipToggleSx}>
            ...
            {form.ipMode === "custom" && <TextField value={form.ipList} onChange={(e) => setForm((f) => ({ ...f, ipList: e.target.value }))} fullWidth multiline minRows={2} placeholder={t("pages.settings.api_keys.fields.ip_placeholder")} helperText={t("pages.settings.api_keys.fields.ip_helper")} sx={fieldSx} />}
          </Box>
        </>}>
      ...
      <Dialog open={!!editingKey} onClose={() => setEditingKey(null)} maxWidth="sm" fullWidth PaperProps={{ sx: { borderRadius: "16px" } }}>
        ...
        <Divider />
        <DialogForm fields={<>
          <Box><FieldLabel text={t("pages.settings.api_keys.fields.key_name")} /><TextField value={editForm.name} onChange={(e) => setEditForm((f) => ({ ...f, name: e.target.value }))} fullWidth sx={fieldSx} /></Box>
          <Box><FieldLabel text={t("pages.settings.api_keys.fields.service_name")} /><TextField value={editForm.serviceName} onChange={(e) => setEditForm((f) => ({ ...f, serviceName: e.target.value }))} fullWidth sx={fieldSx} /></Box>
          <Box><FieldLabel text={t("pages.settings.api_keys.fields.scopes")} /><ScopeChips /></Box>
          <Box sx={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 2 }}>
            <Box><FieldLabel text={t("pages.settings.api_keys.fields.rate_limit")} /><TextField type="number" value={editForm.rateLimit} onChange={(e) => setEditForm((f) => ({ ...f, rateLimit: Number(e.target.value) }))} fullWidth inputProps={{ min: 1 }} sx={fieldSx} /></Box>
          ...
          <Box>
            <FieldLabel text={t("pages.settings.api_keys.fields.ip_whitelist")} />
            <ToggleButtonGroup exclusive size="small" value={editForm.ipMode} onChange={(_, v) => v && setEditForm((f) => ({ ...f, ipMode: v }))} sx={ipToggleSx}>
              <ToggleButton value="all">{t("pages.settings.api_keys.fields.ip_all")}</ToggleButton>
              <ToggleButton value="custom">{t("pages.settings.api_keys.fields.ip_custom")}</ToggleButton>
            </ToggleButtonGroup>
            {editForm.ipMode === "custom" && <TextField value={editForm.ipList} onChange={(e) => setEditForm((f) => ({ ...f, ipList: e.target.value }))} fullWidth multiline minRows={2} placeholder={t("pages.settings.api_keys.fields.ip_placeholder")} helperText={t("pages.settings.api_keys.fields.ip_helper")} sx={fieldSx} />}
          </Box>
        </>}>
      ...
      <Dialog open={!!deleteTarget} onClose={() => setDeleteTarget(null)} maxWidth="xs" fullWidth PaperProps={{ sx: { borderRadius: "16px" } }}>
        ...
        <Divider />
        <DialogContent sx={{ pt: 2.5 }}>
          <Typography sx={{ fontSize: "0.88rem", color: "#374151" }}>{t("pages.settings.api_keys.delete_confirm", { name: deleteTarget?.name })}</Typography>
          <Typography sx={{ fontSize: "0.78rem", color: "#9CA3AF", mt: 0.75 }}>{t("pages.settings.api_keys.delete_warning")}</Typography>
        </DialogContent>
        <Divider />
