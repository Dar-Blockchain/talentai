...
import { usePostAssessmentQuery } from "../queries/usePostAssessmentQuery";
      ...
      <DialogContent sx={{ px: 0, py: 0, overflowY: "auto", flex: 1, display: "flex", flexDirection: "column" }}>
        ...
        {!isLoading && isError && (
          <Box sx={{ display: "flex", flexDirection: "column", alignItems: "center", py: 10, gap: 1 }}>
            ...
            <Typography sx={{ fontSize: "0.73rem", color: "#9CA3AF" }}>{(error as Error)?.message || ""}</Typography>
          </Box>
        )}
