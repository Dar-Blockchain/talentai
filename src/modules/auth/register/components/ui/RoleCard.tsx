...
const RoleCard: React.FC<Props> = ({ icon, label, description, accent, iconGradient, shadowColor, border, onClick }) => (
  <div
    onClick={onClick}
    className={cn(
      "group flex items-center gap-4 sm:gap-5 p-4 sm:p-5 rounded-2xl border-[1.5px] bg-card cursor-pointer",
    ...
    onMouseEnter={(e) => {
      ...
      el.style.boxShadow   = `0 10px 36px ${shadowColor}, 0 2px 8px rgba(0,0,0,0.04)`;
    }}
    onMouseLeave={(e) => {
      const el = e.currentTarget;
  ...
  >
    ...
    {/* Arrow */}
    <div
      className="w-7 h-7 sm:w-8 sm:h-8 rounded-md flex items-center justify-center shrink-0 transition-colors duration-200"
      style={{ background: `${accent}10`, border: `1px solid ${accent}20` }}
    >
      <ArrowRight className="size-3.5 sm:size-4 transition-transform duration-200 group-hover:translate-x-0.5" style={{ color: accent }} />
    </div>
  </div>
);
