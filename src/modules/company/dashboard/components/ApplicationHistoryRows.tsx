...
import { UserPlus as UserPlusOutlined, Mail as MailOutlined, CheckCircle2 as CheckCircleOutlined, Award as AwardOutlined, XCircle as XCircleOutlined, SearchX as SearchXOutlined } from "lucide-react";
...
const ApplicationHistoryRows = memo<Props>(({ items, showRail = true }) => {
  ...
  return (
    <TooltipProvider>
      {items.map((item, idx) => {
        ...
        const clickable = item.hasInterview && !!item.candidateUserId;
        ...
        const content = (
          <>
            <div className="relative flex flex-col items-center shrink-0 self-stretch">
              ...
              {showRail && !isLast && <div className="w-px flex-1 bg-slate-100 mt-1" />}
            </div>
...
ApplicationHistoryRows.displayName = "ApplicationHistoryRows";
export default ApplicationHistoryRows;
