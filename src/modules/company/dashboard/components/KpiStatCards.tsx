...
const KpiStatCards = memo(() => {
  ...
  ), [campaignMetrics?.total, campaignMetrics?.active, t]);
  ...
  return (
    <Box sx={GRID_SX}>
      ...
      <StatCard icon={PeopleOutlined}     color="#8B5CF6"  bg="#F5F3FF" loading={appMetricsLoading} value={appMetrics?.totalApplicants ?? 0}     label={t("overview.stat.applicants")}          onClick={goToApplications} />
      <StatCard icon={CampaignOutlined}   color="#F59E0B"  bg="#FFF7ED" loading={campaignLoading}   value={campaignValue}                        label={t("overview.stat.campaigns")}           onClick={goToCampaigns} />
      <StatCard icon={AccountTreeOutlined} color="#3B82F6" bg="#EFF6FF" loading={deptLoading}       value={deptStats?.total ?? 0}                label={t("overview.stat.departments")}         onClick={goToDepartments} />
      <StatCard icon={GroupsOutlined}     color="#A855F7"  bg="#FDF4FF" loading={memberLoading}     value={memberStats?.total ?? 0}              label={t("overview.stat.team_members")}        onClick={goToEmployees} />
    </Box>
  );
});
