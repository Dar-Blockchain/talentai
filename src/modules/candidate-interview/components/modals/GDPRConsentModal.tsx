...
const GDPRConsentModal: React.FC<GDPRConsentModalProps> = ({ open, onAccept, onDecline }) => {
  ...
  return (
    <Modal open={open} disableEscapeKeyDown>
      <Box sx={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', width: { xs: '94vw', sm: 540 }, bgcolor: '#fff', borderRadius: '16px', border: '1px solid #E5E7EB', boxShadow: '0 20px 60px rgba(0,0,0,0.12)', overflow: 'hidden', outline: 'none' }}>

        <Box sx={{ px: 3, py: 2.5, bgcolor: PURPLE, display: 'flex', alignItems: 'center', gap: 1.5 }}>
          <Box sx={{ width: 40, height: 40, borderRadius: '10px', bgcolor: 'rgba(255,255,255,0.18)', border: '1px solid rgba(255,255,255,0.25)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
            <LockOutlinedIcon sx={{ fontSize: 20, color: '#fff' }} />
          </Box>
          <Box>
            <Typography sx={{ fontFamily: 'Poppins', fontWeight: 700, fontSize: '1rem', color: '#fff', lineHeight: 1.25 }}>{t('gdpr.privacy_title')}</Typography>
            <Typography sx={{ fontFamily: 'Poppins', fontSize: '0.78rem', color: 'rgba(255,255,255,0.82)', mt: 0.2 }}>{t('gdpr.integrity_title')}</Typography>
          </Box>
        </Box>

        <Box sx={{ px: 3.5, pt: 2.5, pb: 2 }}>
          <Typography sx={{ fontFamily: 'Poppins', fontSize: '0.82rem', color: '#374151', lineHeight: 1.75, mb: 2 }}>{t('gdpr.body')}</Typography>
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.25, mb: 2.5 }}>
            {[t('gdpr.bullet_data'), t('gdpr.bullet_privacy')].map((text) => (
              <Box key={text} sx={{ display: 'flex', alignItems: 'flex-start', gap: 1 }}>
                ...
                <Typography sx={{ fontFamily: 'Poppins', fontSize: '0.8rem', color: '#374151', lineHeight: 1.7 }}>{text}</Typography>
              </Box>
            ))}
          </Box>

          <Box sx={{ bgcolor: '#F9FAFB', border: '1px solid #E5E7EB', borderRadius: '10px', px: 2.5, py: 1.75 }}>
            <Typography sx={{ fontFamily: 'Poppins', fontSize: '0.78rem', color: '#4B5563', lineHeight: 1.65 }}>{t('gdpr.confirmation')}</Typography>
          </Box>
        </Box>

        <Box sx={{ px: 3.5, py: 2, borderTop: '1px solid #F3F4F6', display: 'flex', gap: 1.25, justifyContent: 'flex-end' }}>
