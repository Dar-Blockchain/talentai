...
import { SX, PURPLE, PURPLE_DARK, PURPLE_BG, PURPLE_BORDER } from "../../styles/interviewIntro.styles";
...
const InterviewIntro: React.FC<InterviewIntroProps> = ({ interviewConfig, jobData, onNext }) => {
  ...
  const tips = [t("intro.tip1"), t("intro.tip2"), t("intro.tip3"), t("intro.tip4")];
        ...
        <Container maxWidth="md" sx={{ py: { xs: 3, md: 4 } }}>
          ...
          <Box sx={{ display: "flex", alignItems: "center", mb: 3.5 }}>
            <Box sx={{ display: "flex", alignItems: "center", gap: 1, flexShrink: 0 }}>
              ...
              <Typography sx={SX.stepLabelActive}>{t("intro.step1_label")}</Typography>
            </Box>
            ...
            <Box sx={{ display: "flex", alignItems: "center", gap: 1, flexShrink: 0 }}>
              ...
              <Typography sx={SX.stepLabelInactive}>{t("intro.step2_label")}</Typography>
            </Box>
          </Box>
          ...
          <Box sx={{ bgcolor: "#fff", borderRadius: "12px", border: "1px solid #E5E7EB", overflow: "hidden" }}>
            ...
            <Box sx={{ px: { xs: 3, md: 4 }, pt: 3, pb: 2.5, borderBottom: "1px solid #F3F4F6" }}>
              ...
              {(location || contractType || workMode || experienceLevel) && (
                <Box sx={{ display: "flex", gap: 1, flexWrap: "wrap", mt: 2 }}>
                  {location      && <Chip icon={<LocationOnOutlinedIcon />}       label={location}        size="small" sx={SX.metaChip} />}
                  {contractType  && <Chip icon={<BusinessCenterOutlinedIcon />}   label={contractType}    size="small" sx={SX.metaChip} />}
                  {workMode      && <Chip                                          label={workMode}        size="small" sx={{ ...SX.metaChip, "& .MuiChip-icon": undefined }} />}
                  {experienceLevel && <Chip                                        label={experienceLevel} size="small" sx={SX.skillChip} />}
                </Box>
              )}
            </Box>
            ...
            <Box sx={{ px: { xs: 3, md: 4 }, py: 3, display: "grid", gridTemplateColumns: { xs: "1fr", md: "1fr 1fr" }, gap: { xs: 3, md: 4 } }}>
              ...
              <Box sx={{ display: "flex", flexDirection: "column", gap: 2.5 }}>
                {description && (
                  <Box>
                    <Typography sx={SX.sectionHeading}>{t("intro.about_role")}</Typography>
                ...
                {skills.length > 0 && (
                  <Box>
                    <Typography sx={SX.sectionHeading}>{t("intro.key_skills")}</Typography>
                    <Box sx={{ display: "flex", flexWrap: "wrap", gap: 0.75 }}>
                      {skills.map((s, i) => <Chip key={i} label={s} size="small" sx={SX.skillChip} />)}
                    </Box>
                  </Box>
                )}
                {!description && !skills.length && (
                  <Box sx={{ display: "flex", alignItems: "center", gap: 1.5, p: 2, bgcolor: "#F9FAFB", borderRadius: "10px", border: "1px solid #F3F4F6" }}>
                    ...
                    <Typography sx={SX.bodyText}>{t("intro.voice_video_desc")}</Typography>
                  </Box>
                )}
              </Box>
              ...
              <Box>
                <Typography sx={{ ...SX.sectionHeading, mb: 1.25 }}>{t("intro.before_start")}</Typography>
                ...
                <Box sx={{ mt: 2.5, display: "flex", gap: 1, flexWrap: "wrap" }}>
                  <Box sx={SX.pill}>
                    ...
                    <Typography sx={SX.pillText}>{t("intro.video_label")}</Typography>
                  </Box>
                  <Box sx={SX.pill}>
                    <MicNoneOutlinedIcon sx={{ fontSize: 14, color: "#6B7280" }} />
                    <Typography sx={SX.pillText}>{t("intro.voice_label")}</Typography>
                  </Box>
            ...
            <Box sx={{ px: { xs: 3, md: 4 }, pb: 3, pt: 0.5, display: "flex", justifyContent: "flex-end", borderTop: "1px solid #F3F4F6" }}>
              <Button
                ...
                onClick={onNext}
                sx={{ bgcolor: PURPLE, color: "#fff", fontFamily: "Poppins", fontWeight: 600, fontSize: "0.86rem", px: 3.5, py: 1.15, borderRadius: "8px", textTransform: "none", boxShadow: "none", minWidth: 160, "&:hover": { bgcolor: PURPLE_DARK, boxShadow: "none" }, "&.Mui-disabled": { bgcolor: "#E5E7EB", color: "#9CA3AF" } }}
              >
                {t("start.btn_start")}
              </Button>
            </Box>
