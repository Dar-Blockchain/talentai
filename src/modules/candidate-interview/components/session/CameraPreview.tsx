        ...
        {cameraStatus !== 'granted' && (
          <Box sx={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 2, background: 'linear-gradient(135deg,#1a1040,#2d1b69)' }}>
            {cameraStatus === 'requesting' ? (
              ...
              <>
                <Box sx={{ width: 64, height: 64, borderRadius: '50%', background: 'rgba(255,255,255,0.07)', border: '1.5px solid rgba(255,255,255,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <VideocamOffIcon sx={{ color: 'rgba(255,255,255,0.4)', fontSize: 28 }} />
                </Box>
                <Typography sx={{ color: 'rgba(255,255,255,0.5)', fontSize: '0.82rem', fontFamily: 'Poppins', textAlign: 'center', px: 3 }}>
        ...
        <Box display="flex" alignItems="center" justifyContent="space-between">
          <Box display="flex" alignItems="center" gap={0.75}>
            <Box sx={{ width: 26, height: 26, borderRadius: '7px', bgcolor: '#f3f4f6', border: '1px solid #e5e7eb', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              {isActive ? <MicIcon sx={{ fontSize: 14, color: '#374151' }} /> : <MicOffIcon sx={{ fontSize: 14, color: '#9ca3af' }} />}
            </Box>
            <Typography sx={{ fontFamily: 'Poppins', fontWeight: 600, fontSize: '0.75rem', color: '#374151' }}>{t('camera.microphone')}</Typography>
          </Box>
          <Box sx={{ px: 1, py: 0.3, borderRadius: '20px', bgcolor: isActive ? 'rgba(34,197,94,0.08)' : '#f3f4f6', border: `1px solid ${isActive ? 'rgba(34,197,94,0.2)' : '#e5e7eb'}` }}>
            <Typography sx={{ fontFamily: 'Poppins', fontSize: '0.62rem', fontWeight: 700, color: isActive ? '#16a34a' : '#9ca3af', textTransform: 'uppercase', letterSpacing: '0.06em' }}>
              {isActive ? t('camera.mic_active') : t('camera.mic_inactive')}
            </Typography>
          </Box>
        </Box>

        <Box sx={{ display: 'flex', alignItems: 'center', gap: '2px', height: 40, borderRadius: '10px', px: 1.5, bgcolor: '#f9fafb', border: '1px solid #e5e7eb' }}>
          {bars.map((h, i) => (
            <Box key={i} sx={{ flex: 1, height: `${h}px`, borderRadius: '3px', background: isActive ? '#374151' : '#d1d5db', transition: 'height 0.05s ease', minWidth: 0 }} />
          ))}
        </Box>
