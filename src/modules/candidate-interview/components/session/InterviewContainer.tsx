...
const REPORT_POLL_INTERVAL = 1500;
const REPORT_MAX_WAIT      = 20000;
    ...
    <Box sx={{ bgcolor: 'transparent', borderRadius: '16px', border: '1px solid #ede9f8', overflow: 'hidden' }}>
      ...
      {interviewStatus === 'active' && (
        <Box sx={{ bgcolor: '#fff', borderBottom: '1px solid #ede9f8', px: 2.5, py: 1.75, display: 'flex', alignItems: 'center', gap: 1.5 }}>
          <Box sx={{ width: 40, height: 40, borderRadius: '50%', flexShrink: 0, bgcolor: agentState === 'thinking' || agentState === 'processing' ? 'rgba(245,158,11,0.1)' : 'rgba(131,16,255,0.08)', border: `1px solid ${agentState === 'thinking' || agentState === 'processing' ? 'rgba(245,158,11,0.2)' : 'rgba(131,16,255,0.15)'}`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          ...
          <Box>
            ...
            <Typography sx={{ fontFamily: 'Poppins', fontSize: '0.72rem', color: '#9ca3af', mt: 0.2 }}>
              {agentState === 'thinking' || agentState === 'processing' ? t('container.wait') : t('container.listening')}
            </Typography>
          </Box>
        </Box>
      )}
      ...
      <Box sx={{ p: { xs: 2.5, md: 3.5 } }}>
        ...
        {interviewStatus === 'idle' && (
          <Box sx={{ textAlign: 'center' }}>
            ...
            <Button variant="contained" fullWidth onClick={onStartInterview} disabled={!allReady} startIcon={<PlayArrowIcon />} sx={{ fontFamily: 'Poppins', fontWeight: 700, fontSize: '0.92rem', py: 1.5, borderRadius: '12px', bgcolor: allReady ? '#8310FF' : '#e5e7eb', color: '#fff', textTransform: 'none', boxShadow: 'none', '&:hover': { bgcolor: allReady ? '#6d0ee0' : '#e5e7eb', boxShadow: 'none' }, '&.Mui-disabled': { bgcolor: '#f3f4f6', color: '#9ca3af', boxShadow: 'none' } }}>
              {t('start.btn_start')}
            </Button>
        ...
        {interviewStatus === 'active' && (
          <Box sx={{ py: 1 }}>
            <Box id="transcript-scroll" sx={{ maxHeight: 200, overflowY: 'auto', mb: 2, pr: 0.5, '&::-webkit-scrollbar': { width: 4 }, '&::-webkit-scrollbar-track': { bgcolor: 'transparent' }, '&::-webkit-scrollbar-thumb': { bgcolor: 'rgba(131,16,255,0.2)', borderRadius: 2 } }}
