...
const CoverageDashboard: React.FC<CoverageDashboardProps> = ({ coverage }) => {
  ...
  const scoreLabel = (pct: number) =>
    pct >= 80
      ? t("coverage_dashboard.strong")
      : pct >= 50
        ? t("coverage_dashboard.moderate")
        : t("coverage_dashboard.needs_work");
          ...
          {areas.length > 0 && (
            <Box>
              ...
              <Box sx={{ display: "flex", flexDirection: "column", gap: 0 }}>
                {areas.map(
                  ([areaName, areaData]: [string, CoverageArea], idx) => {
