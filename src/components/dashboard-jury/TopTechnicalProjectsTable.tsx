...
interface TopTechnicalProjectsTableProps {
  data: any[];
  loading: boolean;
  error: string | null;
  page: number;
...
const TopTechnicalProjectsTable: React.FC<TopTechnicalProjectsTableProps> = ({
  data,
  loading,
  error,
  page,
  ...
  <Paper sx={{ flex: 1, p: 2, borderRadius: 4, mb: { xs: 3, md: 0 } }}>
    <Typography variant="h6" fontWeight={700} mb={2}>Top Projects by Technical Score</Typography>
    {loading ? (
          ...
          <Table size="small">
            <TableHead>
              <TableRow>
                ...
                <TableCell sx={{ fontWeight: 700 }}>Technical Score</TableCell>
            ...
            <TableBody>
              {data?.length > 0 && data.map((project: any) => (
                <TableRow key={project._id} hover>
                  ...
                  <TableCell>{(() => {
                    const score = project.assessment?.technicalData?.overallScore;
...
export default TopTechnicalProjectsTable; 