...
const JobPreviewPanel: React.FC<JobPreviewPanelProps> = ({ jobData, jobId, companyId }) => {
  ...
  const skills = getPostSkills(jobData);
  ...
  return (
    <>
      ...
      <Box sx={{ bgcolor: '#F8F9FA', minHeight: 'calc(100vh - 56px)', py: { xs: 3, md: 5 } }}>
        <Box sx={{
          maxWidth: 1100, mx: 'auto', px: { xs: 2, md: 4 },
          display: 'flex', gap: 3,
          ...
          <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 2.5 }}>
            ...
            <SectionCard>
              ...
              <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1 }}>
                ...
                {jobData?.createdAt && <MetaBadge icon={<CalendarTodayOutlinedIcon sx={{ fontSize: 14 }} />} label={formatDate(jobData.createdAt)} color="#6B7280" bg="#F9FAFB" border="#E5E7EB" />}
              </Box>
            </SectionCard>
            ...
            {skills.length > 0 && (
              <SectionCard>
                <Box sx={{ display: 'flex', gap: 2, flexDirection: { xs: 'column', sm: 'row' } }}>
                  {skills.filter((s: Skill) => s.type === 'technical').length > 0 && (
                    <Box sx={{ flex: 1 }}>
                      <SectionTitle icon={<CodeIcon sx={{ fontSize: 15 }} />} title={t('section.technical_skills')} />
                      <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.75 }}>
                        {skills.filter((s: Skill) => s.type === 'technical').map((skill: Skill, i: number) => (
                  ...
                  {skills.filter((s: Skill) => s.type === 'soft').length > 0 && (
                    <Box sx={{ flex: 1 }}>
                      <SectionTitle icon={<PsychologyIcon sx={{ fontSize: 15 }} />} title={t('section.soft_skills')} />
                      <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.75 }}>
                        {skills.filter((s: Skill) => s.type === 'soft').map((skill: Skill, i: number) => (
                          <Chip key={i} size="small"
