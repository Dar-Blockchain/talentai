...
const SkillsHeader: React.FC = () => {
  ...
  return (
    <Box sx={{
      ...
      <Box sx={{ position: "absolute", top: -50, right: -50, width: 180, height: 180, borderRadius: "50%", bgcolor: `${TEAL}07`, pointerEvents: "none" }} />
      ...
      <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 2 }}>
        <Box sx={{ display: "flex", alignItems: "center", gap: 1.5 }}>
          <Box sx={{ width: 42, height: 42, borderRadius: "12px", bgcolor: TBG, border: `1px solid ${TBD}`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
            <PsychologyOutlined sx={{ fontSize: 22, color: TEAL }} />
          </Box>
          <Box>
      ...
      <Box sx={{ mt: 2.5, pt: 2, borderTop: "1px solid #F3F4F6" }}>
        <Box sx={{ display: "flex", justifyContent: "space-between", mb: 0.75 }}>
          ...
          <Typography sx={{ fontSize: "0.72rem", fontWeight: 800, color: quota >= 5 ? "#DC2626" : TEAL }}>{s("quota_used", { used: quota })}</Typography>
        </Box>
        <Box sx={{ height: 6, bgcolor: "#F3F4F6", borderRadius: "99px", overflow: "hidden" }}>
          <Box sx={{ height: "100%", width: `${(quota / 5) * 100}%`, bgcolor: quota >= 5 ? "#DC2626" : TEAL, borderRadius: "99px", transition: "width 0.6s ease" }} />
        </Box>
        {quota >= 5 && (
