...
const FiltersButton: React.FC<{ activeCount: number; onClick: () => void }> = ({ activeCount, onClick }) => (
    ...
    <>
      ...
      <Box sx={{ display: "flex", alignItems: "center", gap: 1, flex: 1, minWidth: 0 }}>
        ...
        {!isDesktop && !isMobile && (
          <>
            <SearchInput value={search} onChange={onSearchChange} />
            <FiltersButton activeCount={activeCount} onClick={() => setDrawerOpen(true)} />
          </>
        )}
        ...
        {isMobile && (
          <>
            <SearchInput value={search} onChange={onSearchChange} placeholder="Search…" py={0.5} />
            <FiltersButton activeCount={activeCount} onClick={() => setDrawerOpen(true)} />
          </>
        )}
      ...
      >
        ...
        <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", px: 2.5, py: 1.75, borderBottom: "1px solid #F3F4F6" }}>
          <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
            ...
            <Typography sx={{ fontWeight: 700, fontSize: "16px", color: "#111827" }}>Filters</Typography>
            {activeCount > 0 && (
              <Box sx={{ px: 1, py: 0.25, borderRadius: 999, bgcolor: `${PURPLE}12`, border: `1px solid ${PURPLE}25` }}>
                <Typography sx={{ fontSize: "11px", fontWeight: 800, color: PURPLE }}>{activeCount} active</Typography>
              </Box>
            )}
          </Box>
        ...
        <Box sx={{ px: 2.5, py: 2, borderTop: "1px solid #F3F4F6", display: "flex", gap: 1.5 }}>
          <Box onClick={clearAll} sx={{ flex: 1, py: 1.25, borderRadius: "12px", textAlign: "center", cursor: "pointer", border: "1px solid #E5E7EB", bgcolor: "#F9FAFB", "&:hover": { bgcolor: "#F3F4F6" }, transition: "all 0.15s" }}>
            <Typography sx={{ fontSize: "14px", fontWeight: 600, color: "#374151" }}>Clear all</Typography>
          </Box>
