...
{{companyName}} Team`,
  },
  {
    ...
    defaultSubject:
      "Interview Invitation - {{positionTitle}} at {{companyName}}",
    defaultBody: `Dear {{candidateName}},
...
const EmailConfigForm: React.FC<EmailConfigFormProps> = ({
  ...
  const [emailType, setEmailType] = useState(
    initialConfig?.emailType || "welcome"
  );
        ...
        <TextField
          ...
          helperText={
            body.trim() === ""
              ...
              : "Use placeholders above to personalize the email"
          }
          sx={{
            ...
            {SEND_TO_OPTIONS.map((option) => (
              <Box
                ...
                sx={{
                  border: "1px solid",
                  borderColor:
                    sendTo === option.value ? "primary.main" : "#e0e0e0",
                  borderRadius: "8px",
                  p: 1.5,
                  backgroundColor:
                    sendTo === option.value ? "#f0f7ff" : "white",
                  cursor: "pointer",
                  transition: "all 0.2s",
                  "&:hover": {
                    backgroundColor:
                      sendTo === option.value ? "#e3f2fd" : "#f5f5f5",
                  },
                }}
