...
const SplitLineConnector = styled(StepConnector)<{ userType?: string }>(({ userType }) => ({
  ...
  [`& .MuiStepConnector-line`]: { position: "relative", height: "8px", border: 0, borderRadius: "8px", backgroundColor: "transparent" },
  [`& .MuiStepConnector-line::before,& .MuiStepConnector-line::after`]: { content: '""', position: "absolute", top: 0, height: "8px", width: "48%", borderRadius: "8px", backgroundColor: "rgba(210, 225, 238, 1)" },
  ...
  [`&.Mui-active .MuiStepConnector-line::before,&.Mui-active .MuiStepConnector-line::after,&.Mui-completed .MuiStepConnector-line::before,&.Mui-completed .MuiStepConnector-line::after`]: {
    backgroundColor: userType === "candidate" ? "rgba(232, 209, 255, 1)" : "rgba(223, 249, 239, 1)",
  },
}));

// ---------- Step Icon ----------
const StepIconRoot = styled("div")<{ ownerState: { active?: boolean; completed?: boolean; userType?: string } }>(({ ownerState }) => ({
  zIndex: 1,
  backgroundColor: ownerState.active || ownerState.completed
    ? ownerState.userType === "candidate" ? "rgba(189, 133, 255, 1)" : "rgba(12, 218, 139, 1)"
    : "rgba(210, 225, 238, 1)",
  color: "#fff",
  display: "flex",
...
function CustomStepIcon(props: any) {
  ...
  return <StepIconRoot ownerState={{ active, completed, userType }} className={className}>
    {completed ? <Check fontSize="small" /> : props.icon}
  </StepIconRoot>;
}

type OnboardingStepperProps = { preferences: ReturnType<typeof import("./hooks/usePreferences").usePreferences> };

const OnboardingStepper: React.FC<OnboardingStepperProps> = ({ preferences }) => {
  ...
  const stepsInfo = userType === "candidate" ? candidateStepsInfo : companyStepsInfo;
  ...
  const handleNext = () => {
    if (userType === "candidate" && activeStep === 1 && !candidateDetailsRef.current?.validate()) return;
    if (userType === "company" && activeStep === 1 && !companyDetailsRef.current?.validate()) return;
    if (userType === "candidate" && activeStep === 2 && !selectSkillsRef.current?.validate()) return;
    if (userType === "company" && activeStep === 2 && !skillStackRef.current?.validate()) return;
  ...
  const handleSaveAndRedirect = async () => {
    ...
    try {
      ...
      if (createOrUpdateProfile.fulfilled.match(resultAction)) {
        ...
        router.push({ pathname: "/interview", query: { type: "on-boarding", skills: skills.join(","), experienceLevel: "Entry Level" } });
      } else {
  ...
  return (
    <Box sx={{ maxWidth: "100%" }}>
      ...
      <Box sx={{ display: "flex", alignItems: "center", justifyContent: "flex-start", gap: 2 }}>
        <Typography variant="h6" sx={{ fontWeight: 600, fontSize: "32px", lineHeight: "24px" }}>
          {stepsInfo[activeStep]?.title}
        </Typography>
        {activeStep === 0 && <Box component="img" src={userType === "candidate" ? "/logo-purple.svg" : "/logo.svg"} alt="Logo" sx={{ height: 36 }} />}
      </Box>
      {stepsInfo[activeStep]?.subtitle && (
        <Typography sx={{ fontSize: "16px", fontWeight: 400, lineHeight: "24px", color: "rgba(140, 140, 140, 1)", mt: 2, mb: 6 }}>
          {stepsInfo[activeStep].subtitle}
        </Typography>
      )}

      {/* Stepper */}
      <Stepper alternativeLabel activeStep={activeStep} connector={<SplitLineConnector userType={userType} />} sx={{ position: "relative", mt: 4, mb: 4 }}>
        {steps.map(label => (
          <Step key={label}>
            <StepLabel StepIconComponent={(props) => <CustomStepIcon {...props} userType={userType} />}>
              <Typography sx={{ fontWeight: 500, fontSize: "15px", lineHeight: "25px", color: activeStep >= steps.indexOf(label) ? (userType === "candidate" ? "rgba(189, 133, 255, 1)" : "rgba(12, 218, 139, 1)") : "rgba(210, 225, 238, 1)" }}>
                {label}
              </Typography>
            </StepLabel>
          </Step>
      ...
      <Box sx={{ display: "flex", flexDirection: "column", alignItems: "flex-start", mt: 4 }}>
        ...
        {activeStep === 1 && (userType === "candidate" ? <CandidateDetails ref={candidateDetailsRef} preferences={preferences} /> : <CompanyDetails ref={companyDetailsRef} preferences={preferences} />)}
        {activeStep === 2 && (userType === "candidate" ? <SelectSkills ref={selectSkillsRef} preferences={preferences} /> : <SkillsStack ref={skillStackRef} preferences={preferences} />)}
        {activeStep === 3 && (userType === "candidate" ? <CandidateReview preferences={preferences} /> : <CompanyReview preferences={preferences} />)}
      </Box>
