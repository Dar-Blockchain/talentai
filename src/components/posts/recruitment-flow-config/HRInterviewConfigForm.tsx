...
const FOCUS_AREAS = [
  ...
  { value: 'technical_background', label: 'Technical Background Review', description: 'Overview of technical skills' },
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
            ...
            <Box
              sx={{
                ...
                transition: 'all 0.2s',
                '&:hover': {
                  backgroundColor: interviewMode === 'ai' ? '#bbdefb' : '#f5f5f5',
                ...
                label={
                  <Box>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 0.5 }}>
                      <SmartToyIcon color={interviewMode === 'ai' ? 'primary' : 'disabled'} />
                      <Typography variant="body1" fontWeight={interviewMode === 'ai' ? 600 : 400}>
                        🤖 AI-Powered Interview
                      </Typography>
                    </Box>
                    <Typography variant="caption" color="textSecondary" sx={{ pl: 4 }}>
                      Automated interview conducted by AI agent based on focus areas
                    </Typography>
                  </Box>
                }
              />
            ...
            <Box
              sx={{
                ...
                borderRadius: '12px',
                p: 2,
                backgroundColor: interviewMode === 'human' ? '#e3f2fd' : 'white',
                cursor: 'pointer',
                transition: 'all 0.2s',
                '&:hover': {
                  backgroundColor: interviewMode === 'human' ? '#bbdefb' : '#f5f5f5',
                  transform: 'translateX(4px)',
                },
              }}
            ...
            >
              <FormControlLabel
                ...
                label={
                  <Box>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 0.5 }}>
                      <PersonIcon color={interviewMode === 'human' ? 'primary' : 'disabled'} />
                      <Typography variant="body1" fontWeight={interviewMode === 'human' ? 600 : 400}>
                        👤 Human Interview
                      </Typography>
                    </Box>
                    <Typography variant="caption" color="textSecondary" sx={{ pl: 4 }}>
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
            {INTERVIEW_TYPES.map((type) => (
              <Box
                ...
                sx={{
                  border: '1px solid',
                  borderColor: interviewType === type.value ? 'primary.main' : '#e0e0e0',
                  borderRadius: '8px',
                  p: 1.5,
                  backgroundColor: interviewType === type.value ? '#f0f7ff' : 'white',
                  cursor: 'pointer',
                  transition: 'all 0.2s',
                  '&:hover': {
                    backgroundColor: interviewType === type.value ? '#e3f2fd' : '#f5f5f5',
                    transform: 'translateX(4px)',
                  },
                }}
      ...
      {focusAreas.length > 0 && (
        ...
        >
          ...
          <Box sx={{ mb: 1.5 }}>
            ...
            <Chip
              label={interviewMode === 'ai' ? '🤖 AI-Powered Interview' : '👤 Human Interview'}
          ...
          {interviewMode === 'human' && (
            <Box sx={{ mb: 1.5 }}>
              ...
              <Chip
                label={INTERVIEW_TYPES.find((t) => t.value === interviewType)?.label}
                icon={INTERVIEW_TYPES.find((t) => t.value === interviewType)?.icon}
                color="primary"
                size="small"
              />
            </Box>
