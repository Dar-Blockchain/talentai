...
const AssessmentHeader: React.FC<AssessmentHeaderProps> = ({ assessment }) => {
  const candidateName = assessment?.candidate?.username || 'Unknown User';
    ...
    <Box sx={sectionStyle}>
      ...
      <Box sx={{ mt: 2 }}>
        ...
        <Stack direction="row" spacing={0.5} sx={{ flexWrap: 'wrap', gap: 0.5 }}>
          ...
          <InfoChip
            label={new Date(timestamp).toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric' })}
      ...
      <Box sx={{ mt: 3 }}>
        ...
        <Box sx={{ display: 'grid', gridTemplateColumns: { xs: 'repeat(2, 1fr)', md: 'repeat(4, 1fr)' }, gap: 2 }}>
          <StatBox icon={<AccessTimeIcon />} value={formatDuration(analytics.duration || 0)} label="Duration" color="#6366f1" />
          <StatBox icon={<ChatIcon />} value={analytics.messageCount || 0} label="Messages" color="#10b981" />
          <StatBox icon={<AssessmentIcon />} value={`${analytics.completedAreas || 0}/${analytics.totalAreas || 4}`} label="Areas Covered" color="#f59e0b" />
          <StatBox icon={<TrendingUpIcon />} value={`${Math.round(analytics.coveragePercentage || 0)}%`} label="Coverage" color="#8b5cf6" />
        </Box>
      </Box>
    </Box>
  );
