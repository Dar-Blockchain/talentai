...
const INTERVIEW_TYPES = [
    { label: "Post Interview", value: "post_interview", icon: <AssignmentTurnedInIcon />, color: "#667eea" },
    { label: "Onboarding", value: "onboarding", icon: <CalendarTodayIcon />, color: "#f093fb" },
    { label: "HR", value: "hr", icon: <PersonOutlineIcon />, color: "#4facfe" },
    { label: "Technical", value: "skill", icon: <TrendingUpIcon />, color: "#43e97b" },
    { label: "Soft Skills", value: "soft", icon: <PsychologyIcon />, color: "#fa709a" },
];
...
export default function InterviewDetailsModern({ profile }: InterviewDetailsTabsProps) {
    ...
    const fetchData = useCallback(
        async (type: string, signal?: AbortSignal) => {
            ...
            try {
                ...
                const inferredTotal = json.total !== undefined ? json.total : results.length;
    ...
    const getScoreColor = (score: number) => {
        if (score >= 80) return { gradient: "linear-gradient(135deg, #667eea 0%, #764ba2 100%)", bg: "#f3e7ff" };
        if (score >= 70) return { gradient: "linear-gradient(135deg, #4facfe 0%, #00f2fe 100%)", bg: "#e3f2fd" };
        if (score >= 60) return { gradient: "linear-gradient(135deg, #43e97b 0%, #38f9d7 100%)", bg: "#e8f5e9" };
        return { gradient: "linear-gradient(135deg, #fa709a 0%, #fee140 100%)", bg: "#fff3e0" };
    };
                            ...
                            <Box>
                                ...
                                >
                                    <Typography variant="h5" sx={{ fontWeight: 700, color: "#1a1a1a" }}>
                                        {INTERVIEW_TYPES.find((t) => t.value === normalizedTab)?.label} Assessments
                                    </Typography>
                                    <Chip
                                ...
                                {data.length === 0 ? (
                                    ...
                                    >
                                        ...
                                        >
                                            <AssignmentTurnedInIcon sx={{ fontSize: 40, color: "#667eea" }} />
                                        </Avatar>
                                    ...
                                    <Grid container spacing={3}>
                                        {data.map((row: any, index) => {
                                            ...
                                            const score = coverage.overall !== undefined
                                                ? overallCoverage 
                                            ...
                                            const title = skillName || row.post?.jobDetails?.title || "Skill Assessment";
                                                            ...
                                                            <Stack spacing={2}>
                                                                ...
                                                                {dateLabel && (
                                                                    <Stack direction="row" spacing={1} alignItems="center">
                                                                        <CalendarTodayIcon sx={{ fontSize: 16, color: "#9e9e9e" }} />
                                                                        <Typography variant="body2" color="textSecondary">
                                                                            {dateLabel}
                                                                        </Typography>
                                                                    </Stack>
