...
export const fetchRecommendedPosts = createAsyncThunk(
  ...
  async (params: { page?: number; limit?: number } = {}, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(error.response?.data?.message || error.message || "An error occurred while fetching recommended posts");
    }
  }
);
...
export const postRecruitmentSteps = createAsyncThunk(
  ...
  async ({ postId, steps }: { postId: string; steps: any[] }, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(error.response?.data?.message || error.message || "An error occurred while posting recruitment steps");
    }
  }
);
...
export const fetchMyPosts = createAsyncThunk(
  ...
  async (params: { page?: number; limit?: number; search?: string; sort?: string; status?: string; creationType?: string; } = {}, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(error.response?.data?.message || error.message || "An error occurred while fetching posts");
    }
  }
);
...
export const deletePost = createAsyncThunk(
  ...
  async (jobId: string, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(error.response?.data?.message || error.message || "An error occurred while deleting post");
    }
  }
);
...
export const fetchJobById = createAsyncThunk(
  ...
  async (jobId: string, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(error.response?.data?.message || error.message || "An error occurred while fetching the job");
    }
  }
);
    ...
    builder
      ...
      .addCase(postRecruitmentSteps.fulfilled, (state, action) => {
        ...
        if (action.payload.data) {
          ...
          if (state.currentJob) state.currentJob.PostSteps = action.payload.data;
        }
      })
      ...
      .addCase(updatePostStatus.fulfilled, (state, action) => {
        ...
        if (state.currentJob) state.currentJob.status = action.payload.data?.status;
      })
...
export const selectSavePostError   = (state: { post: PostState }) => state.post.savePost.error;
export const selectSavedPost       = (state: { post: PostState }) => state.post.savePost.savedPost;
export const selectSavedPostId     = (state: { post: PostState }): string | null =>
  (state.post.savePost.savedPost?.jobData?._id as string)
  ?? (state.post.savePost.savedPost?.data?._id as string)
  ?? (state.post.savePost.savedPost?._id as string)
  ?? null;

// Candidate assessment selectors
