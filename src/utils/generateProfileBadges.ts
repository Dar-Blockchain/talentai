...
export function generateBadgesFromProfile(
  ...
  // Generate technical badges
  const technicalBadges = generateBadgesForType(technicalEvidence, 'technical');

  // Generate soft skill badges
  const softBadges = generateBadgesForType(softEvidence, 'soft');
...
function generateBadgesForType(
  ...
  evidenceMap.forEach((evidence, skillName) => {
