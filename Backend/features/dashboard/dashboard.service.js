...
module.exports.getStatsCards = async (userId, postId = null, dateFrom = null) => {
  try {
    const scoreMatch = { company: new mongoose.Types.ObjectId(userId), archived: { $ne: true }, "interviewData.finalReport.coverage.overall": { $ne: null } };
    if (postId) scoreMatch.post = new mongoose.Types.ObjectId(postId);
    if (dateFrom) scoreMatch.createdAt = { $gte: new Date(dateFrom) };
