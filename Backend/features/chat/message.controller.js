...
module.exports.sendMessage = async (req, res) => {
  try {
    ...
    const message = await messageService.sendMessage(conversationId, senderId, receiverId, text, { type, attachment, replyTo });
  ...
  try {
    ...
    try {
      ...
      if (result.scope === 'everyone') {
        ...
        const targets = new Set([senderId, receiverId].filter((id) => typeof id === 'string' && id.length > 0));
        targets.forEach((participantId) => {
          chatNamespace.to(`user:${participantId}`).emit('message_updated', payload);
        });
      } else {
        chatNamespace.to(`user:${userIdStr}`).emit('message_deleted', { messageId, conversationId, scope: 'me', deletedBy: userIdStr });
      }
    } catch (socketError) {
      console.error('Error emitting WebSocket event:', socketError);
    }
    ...
    res.status(error.status || 500).json({ success: false, message: error.message || 'Failed to delete message' });
  }
};
