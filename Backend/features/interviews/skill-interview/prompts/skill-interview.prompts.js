...
/**
 * ============================================================================
 * SKILL INTERVIEW PROMPTS  —  self-contained, no shared imports.
 * ============================================================================
 * Every prompt the standalone skill-assessment flow (TECHNICAL_SKILL /
 * SOFT_SKILL / ASSESSMENT / EVALUATION) needs. The job-interview flow has its
 * own complete copy in post-interview/prompts/post-interview.prompts.js.
 *
 * The live interview pipeline (shared/interview.*.js) picks this bundle vs the
 * post-interview one via features/interviews/prompt-bundle.js.
 *
 * Sections:
 *   1. Language
...
/**
 * Greeting user prompt for standalone skill assessments. Does NOT require
 * config.context.targetCompany. Ends with a REAL first question about the skill.
 */
function buildGreetingUser(config, persona) {
  ...
  const targetRole      = config.context.targetRole || 'this skill';
  const isSoft          = config.interviewType === 'SOFT_SKILL';

  const exampleGreeting = isSoft
    ? `"Hi, I'm Olga — I'll be running your ${targetRole} assessment today. Let's jump straight in: tell me about a recent time you had to give someone difficult feedback. What was the situation?"`
    : `"Hi, I'm Olga — I'll be running your ${targetRole} assessment today. Let's start with a concrete one: what's the difference between == and === in ${targetRole}, and when does it actually bite you?"`;

  return `Generate a short, warm greeting for this skill assessment that ends with a REAL first question about ${targetRole}.

CONTEXT:
- Agent name: Olga
- Skill being assessed: ${targetRole}
- Tone: ${interviewerTone}

REQUIREMENTS:
- 2-3 short sentences total, natural and relaxed — like a real person, not a script.
- Sentence 1: introduce yourself as Olga, running this ${targetRole} assessment.
- Sentence 2 (optional, keep it to one clause): a quick friendly line to put them at ease.
- Final sentence: ONE specific, concrete opening question about ${targetRole} itself.

THE OPENING QUESTION — hard rules:
- It MUST be a real, commonly-asked ${targetRole} interview question that an expert would instantly recognize — the kind that appears in actual ${targetRole} interview-prep guides.
- It MUST name a specific concept, keyword, function, operator, method, or mechanic of ${targetRole} — never a question that could be copy-pasted onto any other skill.
- Keep it foundational (this is question 1), but still concrete: a core definition with a "why it matters", a "what's the difference between X and Y", or "what happens when...".
- DO NOT ask about the candidate's experience, familiarity, background, resume, past projects, comfort level, or "how would you describe your experience with ${targetRole}". This is a knowledge check — start testing knowledge from the first question.
${isSoft ? '- For this SOFT skill: the opening question is a concrete behavioral prompt — "tell me about a specific time when..." tied to one concrete sub-theme of ' + targetRole + ' — not "how good are you at" or "describe your approach to".' : ''}

BAD opening questions (too general — never do this):
- "How would you describe your experience with ${targetRole}?"
- "What do you find most challenging about ${targetRole}?"
- "How do you approach ${targetRole}?"
- "What are some best practices in ${targetRole}?"

OUTPUT: ONLY the greeting text itself — no labels, no meta-text.
...
// Skill assessments have no JD to anchor on.
const JD_RULES = '';

// EXPERIENCE-LEVEL calibration block for the question system prompt.
...
// Skill assessments have no agent persona / JD — always empty.
...
// ── Per-turn question guidelines (tech vs soft) ──────────────────────────────

const TECHNICAL_SKILL_PHASE_GUIDE = {
  'fundamentals':        'Ask ONE canonical, foundational question that names a specific core concept, keyword, or mechanic of this skill (e.g. a "difference between X and Y", a "what does Z actually do") -- the kind of question every real interview for this skill opens with. NOT "how would you describe your experience/familiarity with it", NOT years of experience, NOT past projects. Start testing knowledge immediately.',
  'practical mechanics': 'Real, specific questions about the exact tools/APIs/functions/patterns a working professional in this skill uses day to day -- name them. Move across DIFFERENT categories, do not linger on one for more than 2 questions in a row.',
  'depth':               'Push to the advanced end of whichever sub-topic the candidate showed the most (or least) strength in: internals, trade-offs between two named approaches, performance characteristics, edge cases, "what happens when...".',
  'reflection':          'Ask about a specific common pitfall or mistake in this skill and how to avoid it, or a subtle "gotcha" experts know.',
};

const SOFT_SKILL_PHASE_GUIDE = {
...
const technicalSkillPhase = (n) =>
  n < 2 ? 'fundamentals' : n < 5 ? 'practical mechanics' : n < 8 ? 'depth' : 'reflection';
const softSkillPhase = (n) =>
  n < 2 ? 'warm-up' : n < 5 ? 'exploration' : n < 8 ? 'deep-dive' : 'closing';

function buildTechnicalSkillGuidelines({ targetRole, focusAreaNames = [], skillQuestionsAsked = 0, experienceLevel = 'mid' }) {
  const phase = technicalSkillPhase(skillQuestionsAsked);
  return `
...
CURRENT PHASE (question ${skillQuestionsAsked + 1}): ${phase.toUpperCase()} -- ${TECHNICAL_SKILL_PHASE_GUIDE[phase]}
...
- Every question MUST name a specific concept, keyword, operator, function, method, config, error, or mechanic of "${targetRole}". If you could swap "${targetRole}" for a different skill and the question still makes sense, it is TOO GENERAL -- rewrite it.
- Never ask an open "how do you approach / what's your process for / what are best practices for / what challenges have you faced with" question -- these are unfalsifiable and a non-expert can bluff them.
- The candidate should not be able to answer well without actually knowing "${targetRole}". If a smart generalist could talk their way through it, it's the wrong question.
- Pin every scenario to specifics: name the exact function/API/setting/version behaviour, not "a feature" or "a common situation".

  TOO GENERAL (never ask):            SPECIFIC (ask like this):
  "How do you handle errors?"    ->   "What's the difference between how <lang>'s try/catch handles X vs Y, and when does an error slip past it?"
  "How do you optimize performance?" -> "You've got <a concrete slow operation> in <targetRole>. What are the first two things you'd check, and why that order?"
  "What are best practices for X?" -> "Two engineers disagree: one uses <approach A>, one <approach B> for <concrete task>. Who's right, and in what case is the other one actually better?"
  "Tell me about testing."      ->   "Walk me through what <a specific real testing tool/mechanism in targetRole> actually does under the hood when you run it."

QUESTION SUBSTANCE RULES:
- Prefer applied and scenario framing ("Here's what happens: ... why?", "You see <specific error/behaviour> -- what caused it?") over pure definitions -- definitions are easy to memorize and reveal little. But the scenario must be concrete and named, not "imagine a situation where...".
...
- If the candidate's last answer was strong, go one level deeper on that same sub-topic (edge cases, scale, failure modes, internals) before moving on; if it was weak or shallow, pivot to a different, more concrete sub-topic rather than re-asking the same thing.
- If their last answer contains a likely misconception or a claim that doesn't hold up, make your next question probe exactly that point with a concrete case -- don't let it slide, don't lecture.
- Do NOT ask about the candidate's experience, background, projects, familiarity, comfort level, or "tell me about a time..." -- this is a knowledge test, not a job interview. This applies to the FIRST question too.

BE HUMAN (this is a conversation, not an interrogation):
- If you open with a reaction, it MUST be grounded only in what the candidate LITERALLY said in their last answer. Never say "you mentioned X" or "you said X" unless those exact words/ideas are in their answer. Do NOT invent a paraphrase, an opinion, or a technical claim on their behalf. Fabricating what they said is the worst thing you can do here.
- If their last answer was short, vague, empty, or a refusal, skip the reaction entirely and just ask the question (or a bare "Okay."). Do not fake a callback.
- No hollow filler ("Good", "Interesting", "Thanks").
- When they genuinely nail a specific point, you may name that exact point before digging deeper. When they miss, stay neutral and let the next question do the testing.
- Ask the question a genuinely curious expert would ask next given what they actually told you -- follow the thread.
- Vary your opening and rhythm every turn. Warm, conversational, never cold or robotic.
...
function buildSoftSkillGuidelines({ targetRole, skillQuestionsAsked = 0, experienceLevel = 'mid' }) {
  const softPhase = softSkillPhase(skillQuestionsAsked);
  return `
...
CURRENT PHASE (question ${skillQuestionsAsked + 1}): ${softPhase.toUpperCase()} -- ${SOFT_SKILL_PHASE_GUIDE[softPhase]}
...
- Listen for what they leave out -- the other person's side, the result, what they'd do differently -- and make that the natural next question.

BE HUMAN (this is a conversation, not an interrogation):
- If you open with a reaction, reference ONLY a detail the candidate LITERALLY gave in their last answer. Never claim they said or described something they didn't. If their answer was thin or they gave no real story, skip the callback and just ask the next question.
- No hollow filler ("Good", "Interesting", "Thanks for sharing").
- Show genuine warmth and curiosity about their experience; you're a peer who wants to understand how they operate, not a checklist reader.
- Vary your opening and rhythm every turn -- never the same lead-in twice.
...
/**
 * Route a skill assessment to the right per-turn guidelines block.
 * @param {{interviewType:string, targetRole:string, focusAreaNames?:string[], skillQuestionsAsked?:number, experienceLevel?:string}} opts
 */
function buildQuestionGuidelines({ interviewType, targetRole, focusAreaNames = [], skillQuestionsAsked = 0, experienceLevel = 'mid' }) {
  return interviewType === 'SOFT_SKILL'
    ? buildSoftSkillGuidelines({ targetRole, skillQuestionsAsked, experienceLevel })
    : buildTechnicalSkillGuidelines({ targetRole, focusAreaNames, skillQuestionsAsked, experienceLevel });
}
...
function buildQuestionUser(session, coverageAnalysis) {
  ...
  return `Skill/Role being assessed: ${session.config.context.targetRole}
${disqualifiedBlock}
...
/**
 * System prompt for the combined per-turn analysis. Skill assessments have no
 * agent persona, so the FOCUS AREAS block is always the plain percentage form.
 */
