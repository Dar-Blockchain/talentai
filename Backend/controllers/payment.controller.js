...
const { Client, AccountId, TransactionId } = require("@hashgraph/sdk");
...
module.exports.initiateTaiPurchase = async (req, res) => {
  try {
    ...
    console.log(
      `💰 Initiating TAI purchase for user ${userId}, plan: ${planId}`,
    );
    ...
    if (!user.hederaAccountId && user.role === "Company") {
      ...
      try {
        ...
        console.log(
          `✅ Hedera account created for user: ${hederaAccount.hederaAccountId}`,
        );
      } catch (hederaError) {
...
module.exports.verifyHbarPayment = async (req, res) => {
  try {
    ...
    try {
      ...
      const transactionRecord = await new TransactionId.fromString(
        hederaTransactionId,
      ).getRecord(client);
        ...
        await TokenTransaction.findByIdAndUpdate(transaction._id, {
          ...
          metadata: {
            ...
            hederaRecord: {
              ...
              consensusTimestamp:
                transactionRecord.consensusTimestamp?.toString(),
...
module.exports.distributeTokens = async (req, res) => {
  try {
    ...
    console.log(
      `🚀 Starting token distribution for transaction ${transactionId}`,
    );
    ...
    try {
      ...
      const distributionResult =
        await taiTokenDistributionService.completeDistribution(
...
module.exports.completePayment = async (req, res) => {
  try {
    ...
    console.log(
      `🎯 Complete payment initiated for user ${userId}, plan: ${planId}`,
    );
    ...
    if (!user || !user.hederaAccountId || !user.hederaPrivateKey) {
      return res.status(400).json({
        success: false,
        message:
          "User Hedera account not found. Please create a company profile first.",
      });
    }
...
module.exports.completeStripePayment = async (req, res) => {
  try {
    ...
    console.log(
      `🎯 Complete Stripe payment initiated for user ${userId}, plan: ${planId}, session: ${sessionId}`,
    );
    ...
    if (!planId || !sessionId) {
      return res
        ...
        .json({
          success: false,
          message: "Plan ID and Stripe session ID are required",
        });
    }
    ...
    if (existing) {
      await TokenTransaction.findByIdAndUpdate(existing._id, txData, {
        new: true,
      });
    } else {
    ...
    console.log(
      `✅ Stripe payment processed: granted ${taiTokens} TAI to ${userId}`,
    );
    ...
    return res
      ...
      .json({
        success: false,
        message: "Failed to complete Stripe payment",
        error: error.message,
      });
  }
};
...
async function _processStripeSessionCore(session, planId, userId) {
  ...
  if (!userId) {
    console.warn(
      "Stripe session has no userId and no matching user by email; skipping token grant",
      sessionId,
    );
  ...
  if (!paid) {
    throw new Error(
      `Stripe payment not completed. Session status: ${session.status}`,
    );
  }
  ...
  return {
    success: true,
    message: "Stripe session processed",
    data: { transactionId: sessionId, taiTokens, planName: plan.name },
  };
}
...
module.exports.processStripeSession = async (req, res) => {
  try {
    ...
    if (user.hederaAccountId && user.hederaPrivateKey) {
      ...
      try {
        ...
        if (distributionResult.success) {
          ...
          return res.status(207).json({
            success: false,
            message:
              "Payment processed but token distribution partially failed",
            data: {
              ...result.data,
              distributionResult,
            },
      ...
      console.log(
        `ℹ️  User has no Hedera account, tokens granted but not distributed on chain`,
      );

      return res.status(200).json({
        success: true,
        message:
          "Stripe payment processed and tokens granted (no Hedera distribution)",
        data: {
          ...result.data,
