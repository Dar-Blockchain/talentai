...
const createAndInscribeProfile = async (req, res) => {
  try {
    ...
    if (result.success) {
      console.log('✅ HCS-11 Profile created and inscribed successfully for:', companyName);
      console.log('🔗 Verification URL:', result.verificationUrl);
      return res.status(200).json(result);
    } else {
      console.log('❌ HCS-11 Profile creation and inscription failed:', result.message);
      return res.status(400).json(result);
    }
...
const createCompanyAgent = async (req, res) => {
  try {
    console.log('🚀 Creating Company Agent with Hedera Account and HCS-11 Profile');
    
    const { companyName, postId, agentPosition, companyDescription, agentConfig } = req.body;

    // Basic validation
    if (!companyName || !postId || !agentPosition || !companyDescription) {
      return res.status(400).json({
        success: false,
        message: 'Company name, post ID, agent position, and company description are required',
        error: 'MISSING_REQUIRED_FIELDS'
      });
    }
    ...
    try {
      ...
      try {
        ...
        console.warn('⚠️  SDK memo method failed, using fallback format:', sdkError.message);
      ...
      client.setOperator(hederaWallet.accountId, PrivateKey.fromString(hederaWallet.privkey));
      ...
      console.warn('⚠️  Warning: Failed to set HCS-11 memo:', memoError.message);
      console.warn('   - Agent account may not have proper memo, but continuing...');
    }
    ...
    const companyAgent = {
      ...
      hcs11CustomProfile: {
        ...
        specializedCapabilities: agentConfig?.capabilities || ['RECRUITING', 'CANDIDATE_EVALUATION', 'COMPANY_REPRESENTATION'],
        evaluationFramework: {
