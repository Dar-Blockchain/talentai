...
module.exports.updateAssessmentStatus = async (req, res) => {
  try {
    const { assessmentId } = req.params;
    ...
    const assessment = await postInterviewAssessmentService.updateAssessmentStatus(assessmentId, status);
...
// ========== UPDATE - Update stage ==========
module.exports.updateAssessmentStage = async (req, res) => {
  try {
    const { assessmentId } = req.params;
    ...
    const assessment = await postInterviewAssessmentService.updateAssessmentStage(assessmentId, stage);

    res.status(200).json({
      success: true,
      message: `Assessment stage updated to ${stage}`,
      data: assessment
    });
  } catch (error) {
