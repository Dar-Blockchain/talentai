...
const hederaMirrorNodeService = require("../../services/hederaMirrorNodeService");
...
module.exports.getBalance = async (req, res) => {
  try {
    ...
    console.log(`🔍 Fetching real-time TAI balance for user ${userId}, Hedera account: ${user.hederaAccountId}`);

    // Fetch real-time balance from Hedera Mirror Node
    const mirrorNodeResult = await hederaMirrorNodeService.getAccountTaiBalance(user.hederaAccountId);
    ...
    try {
      console.log("⚠️  Mirror Node failed, falling back to database balance...");
      const userId = req.user._id;
      ...
      res.status(200).json({
        ...
        message: "Token balance retrieved from database (Mirror Node unavailable)",
        error: error.message
      });
    } catch (fallbackError) {
