...
async function addConfig(req, res) {
  try {
    ...
    console.log(`📝 [addConfig] Attempting to add new matching configuration by user: ${userId}`);

    const payload = req.body || {};
    const cfg = await matchingConfigService.addConfig(userId, payload);

    console.log(`✅ [addConfig] Configuration successfully created with ID: ${cfg._id}, by user: ${userId}`);
    ...
    console.error(`❌ [addConfig] Error adding matching config for user: ${req.user && req.user._id}, error:`, err.message);
...
async function updateConfig(req, res) {
  try {
    const userId = req.user && req.user._id;
    console.log(`📝 [updateConfig] Attempting to update matching configuration by user: ${userId}`);
    ...
    console.log(`✅ [updateConfig] Configuration updated successfully by user: ${userId}`);
    return res.json({ success: true, config: cfg });
  } catch (err) {
    console.error(`❌ [updateConfig] Error updating matching config for user: ${req.user && req.user._id}, error:`, err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
}
