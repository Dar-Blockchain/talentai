...
exports.getAgentConfigByAgent = async (req, res) => {
  try {
    if (!isValidMongoId(req.params.agentId)) {
      return res
        .status(400)
        .json({ success: false, error: "Invalid agentId format" });
    }
    const cfg = await agentConfigService.getAgentConfigByAgentId(
      req.params.agentId,
    );
    res.status(200).json({ success: true, data: cfg });
  } catch (err) {
...
exports.getAgentConfigByPost = async (req, res) => {
  try {
    if (!isValidMongoId(req.params.postId)) {
      return res
        .status(400)
        .json({ success: false, error: "Invalid postId format" });
    }
    const cfg = await agentConfigService.getAgentConfigByPostId(
      req.params.postId,
    );
    res.status(200).json({ success: true, data: cfg });
  } catch (err) {
...
exports.updateAgentConfig = async (req, res) => {
  try {
    ...
    if (!req.body || typeof req.body !== "object") {
      return res
        .status(400)
        .json({ success: false, error: "Invalid update payload" });
    }
