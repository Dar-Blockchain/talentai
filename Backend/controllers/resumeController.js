...
exports.createResume = async (req, res) => {
  try {
    ...
    if (!Array.isArray(sections) || sections.length === 0) {
      return res
        .status(400)
        .json({ error: "`sections` doit être un tableau non vide" });
    }
...
exports.regenerate = async (req, res) => {
  try {
    ...
    if (!block || !content) {
      return res
        .status(400)
        .json({ error: "Le bloc et son contenu sont requis" });
    }
    ...
    if (!validBlocks.includes(block)) {
      return res
        ...
        .json({
          error:
            "Bloc invalide. Les blocs valides sont : bio, experience, skills, education, projects",
        });
    }
    ...
    const systemMsg = `As a professional CV writer, your task is to enhance the ${block} section by creating exactly two powerful bullet points.
