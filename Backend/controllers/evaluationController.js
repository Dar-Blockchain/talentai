...
// controllers/profileAnalysisController.js
...
/**
 * Retire la partie où le candidat lit la question
 */
function filterAnswer(answer, question) {
  const q = question.trim();
  const a = answer.trim();
  const idx = a.toLowerCase().indexOf(q.toLowerCase());
  if (idx !== -1) {
    // Couper tout avant la fin de la question
    return a.slice(idx + q.length).trim();
  }
  return a;
}
...
exports.analyzeProfileAnswers = async (req, res) => {
  try {
    ...
    // 2. Nettoyer les réponses pour retirer la lecture de la question
    const cleanedQAs = questions.map(qa => ({
      question: qa.question,
      answer: filterAnswer(qa.answer, qa.question)
    }));

    // 3. Préparer le prompt pour GPT
    const prompt = `
...
${cleanedQAs.map(qa => `Q: ${qa.question}\nA: ${qa.answer}`).join('\n\n')}
...
Based on this ${type} assessment, provide a detailed analysis in the following JSON format ONLY (no additional text):
{
  ...
  "evaluationContext": "Based on ${type} interview standards"
    ...
    // 4. Appel OpenAI pour l’analyse
    const response = await openai.chat.completions.create({
      model: "gpt-4-turbo-preview",
      messages: [
        {
          role: "system",
          content: `You are an expert ${type} interviewer specializing in evaluating developer skills. Analyze both the answers and the progression from their current proficiency levels. Provide detailed, actionable feedback in JSON format only.`
        },
        { role: "user", content: prompt }
      ],
      max_tokens: 1000,
    ...
    // 5. Extraction et parsing du JSON retourné
    let analysis;
    ...
    try {
      ...
      // Tentative de correction basique
      jsonStr = jsonStr
        .replace(/,(\s*[}\]])/g, '$1')
        .replace(/'/g, '"')
        .replace(/\n/g, ' ')
    ...
    // 6. Normalisation et calculs supplémentaires
    const normalized = {
      overallScore: Number(analysis.overallScore) || 0,
      skillAnalysis: analysis.skillAnalysis.map(s => {
        const current = Number(s.currentProficiency) || 0;
        const confidence = Number(s.confidenceScore) || 0;
        let demo;
        if (confidence > 80) demo = current + 1;
        else if (confidence >= 50) demo = current;
        else demo = current - 1;
    ...
    // 7. Mise à jour du profil en base (si nécessaire)
    if (user.profile.overallScore === 0 && type === "technical") {
      await profileService.createOrUpdateProfile(user._id, {
        ...
        skills: normalized.skillAnalysis.map(s => ({
          name: s.skillName,
      ...
      const prev = await profileService.getProfileByUserId(user._id);
      await profileService.createOrUpdateProfile(user._id, {
        overallScore: (prev.overallScore + normalized.overallScore) / 2,
        skills: normalized.skillAnalysis.map(s => ({
          name: s.skillName,
          proficiencyLevel: s.demonstratedProficiency,
          experienceLevel: getExperienceLevel(s.demonstratedProficiency),
      ...
      // logique similaire pour les soft skills...
      await profileService.createOrUpdateProfile(user._id, {
        softSkills: normalized.skillAnalysis.map(s => ({
          name: s.skillName,
          category: s.category || '',
          experienceLevel: getExperienceLevel(s.demonstratedProficiency),
          ScoreTest: s.confidenceScore
    ...
    return res.status(200).json({
      ...
      result: {
        ...
        numberOfQuestions: cleanedQAs.length,
        analysis: normalized
      }
    });
