...
exports.distributeInterviewReward = async (userId, score, interviewId) => {
  try {
    console.log(`🎁 [TokenRewardController] Distributing reward for user ${userId}, score: ${score}`);

    // Validate inputs
    if (!userId || !score === undefined || !interviewId) {
      throw new Error('Missing required parameters: userId, score, or interviewId');
    }
    ...
    console.error('❌ [TokenRewardController] Error distributing reward:', error.message);
    throw error;
  }
};
...
exports.claimInterviewReward = async (req, res) => {
  try {
    ...
    console.log(`🎯 [TokenRewardController] Manual claim request for interview ${interviewId} by user ${userId}`);
    ...
    if (!interview) {
      console.log(`❌ [TokenRewardController] Interview not found: ${interviewId}`);
    ...
    if (interview.candidate.toString() !== userId.toString()) {
      console.log(`⛔ [TokenRewardController] Unauthorized claim attempt by user ${userId} for interview ${interviewId}`);
    ...
    console.log(`✅ [TokenRewardController] No existing claim found - proceeding with distribution`);
    ...
    if (rewardResult.success) {
      console.log(`🎉 [TokenRewardController] Manual claim successful: ${rewardResult.amount} TAI`);
      res.status(200).json({
        success: true,
      ...
      console.log(`❌ [TokenRewardController] Manual claim failed: ${rewardResult.error}`);
      res.status(400).json({
        success: false,
    ...
    console.error('❌ [TokenRewardController] Error in manual claim:', error.message);
    res.status(500).json({
      success: false,
...
exports.getRewardEstimate = async (req, res) => {
  try {
    ...
    console.error('❌ [TokenRewardController] Error getting reward estimate:', error.message);
    res.status(500).json({
      success: false,
...
exports.checkRewardStatus = async (req, res) => {
  try {
    ...
    const status = await interviewRewardService.checkRewardClaimed(userId, interviewId);
    ...
    console.error('❌ [TokenRewardController] Error checking reward status:', error.message);
    res.status(500).json({
      success: false,
      error: 'Internal server error'
    });
