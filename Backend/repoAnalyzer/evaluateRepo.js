...
const evaluateCodeQualityWithAI = async (repoData) => {
  ...
  if (!codeSample || codeSample === "No code sample found") {
    console.warn(
      "No code sample available for code quality. Returning default score."
    );
  ...
  try {
        ...
        messages: [
          {
            role: "system",
            content:
              "You are an expert code reviewer. Evaluate the following code based on its structure, readability, and best practices. Give a short feedback and a score from 1 to 10.",
          },
    ...
    return {
      score: 5,
      feedback: "Error or timeout during code quality evaluation.",
    };
  }
};
...
const evaluateDocumentationWithAI = async (repoData) => {
  ...
  try {
        ...
        messages: [
          {
            role: "system",
            content:
              "You are an expert in documentation evaluation. Assess the following README documentation for clarity, completeness, and usefulness. Give a short feedback and a score from 1 to 10.",
          },
          {
            role: "user",
            content: `Here is the README documentation: ${readme}`,
          },
        ],
        model: "deepseek-ai/DeepSeek-V3",
      },
    ...
    return {
      score: 5,
      feedback: "Error or timeout during documentation evaluation.",
    };
  }
};
...
const evaluateFunctionalityWithAI = async (repoData) => {
  ...
  if (!codeSample || codeSample === "No code sample found") {
    console.warn(
      "No code sample available for functionality. Returning default score."
    );
    return { score: 5, feedback: "No code sample available." };
  }
  try {
        ...
        messages: [
          {
            role: "system",
            content:
              "You are an expert software tester. Evaluate the following code for its functionality, completeness, and reliability. Give a short feedback and a score from 1 to 10.",
          },
          { role: "user", content: `Here is a code sample: ${codeSample}` },
        ],
        model: "deepseek-ai/DeepSeek-V3",
    ...
    return {
      score: 5,
      feedback: "Error or timeout during functionality evaluation.",
    };
  }
};
...
const evaluateInnovationWithAI = async (repoData) => {
  ...
  try {
        ...
        messages: [
          {
            role: "system",
            content:
              "You are an expert in software innovation. Assess the following project for its originality and innovative features based on its documentation. Give a short feedback and a score from 1 to 10.",
          },
          {
            role: "user",
            content: `Here is the README documentation: ${readme}`,
    ...
    return {
      score: 5,
      feedback: "Error or timeout during innovation evaluation.",
    };
  }
};
...
const evaluateUserExperienceWithAI = async (repoData) => {
  ...
  try {
        ...
        messages: [
          {
            role: "system",
            content:
              "You are an expert in user experience. Evaluate the following project for its usability and user-friendliness based on its documentation. Give a short feedback and a score from 1 to 10.",
          },
          {
            role: "user",
            content: `Here is the README documentation: ${readme}`,
    ...
    return {
      score: 5,
      feedback: "Error or timeout during user experience evaluation.",
    };
  }
};
...
const evaluateAIResponse = (responseContent) => {
  ...
  if (
    ...
    return 7;
  if (
    content.includes("above average") ||
    content.includes("decent") ||
    content.includes("6")
  )
    return 6;
  if (
    content.includes("average") ||
    content.includes("adequate") ||
    content.includes("5")
  )
    return 5;
  if (
    content.includes("below average") ||
  ...
  if (
    ...
    content.includes("inadequate") ||
    content.includes("3")
  )
    return 3;
  if (
    content.includes("very bad") ||
...
const getCodeSample = async (repoData) => {
  ...
  console.log(
    `[getCodeSample] Template files for ${projectType}:`,
    templateFiles
  );
  ...
  for (const filePath of templateFiles) {
    ...
    try {
      const fileResponse = await axios.get(
        `${repoData.url}/contents/${filePath}`,
        {
          headers: {
      ...
      console.log(
        `[getCodeSample] File size: ${fileContent.length} characters`
      );
      console.log(
        `[getCodeSample] Code preview (first 200 chars): ${fileContent.substring(
          0,
          200
        )}...`
      );
      ...
      console.log(
        `[getCodeSample] File not found: ${filePath} (${
...
const fetchRepoData = async (owner, repo) => {
  try {
    ...
    try {
      ...
      console.log("checkttttt: ", contribRes.data);
  ...
  try {
    ...
    if (files.includes("package.json")) {
      try {
        const pkgResponse = await axios.get(
          `${repoData.url}/contents/package.json`,
          {
            headers: {
              Authorization: `token ${process.env.GITHUB_TOKEN}`,
            },
        ...
        console.log(
          "[detectProjectTypeFromRepo] Could not read package.json:",
          error.message
        );
      }
    }
    ...
    if (
      ...
      (files.includes("App.js") ||
        files.includes("App.jsx") ||
        files.includes("App.tsx"))
    ) {
      return "react";
    }
    ...
    console.error(
      "[detectProjectTypeFromRepo] Error detecting project type:",
      error.message
    );
    return "custom";
  }
...
const getComprehensiveCodeAnalysis = async (
  ...
  console.log(
    "[getComprehensiveCodeAnalysis] Starting comprehensive file analysis..."
  );
  ...
  if (selectedTemplate === "auto") {
    ...
    console.log(
      `[getComprehensiveCodeAnalysis] Auto-detected project type: ${projectType}`
    );
  } else {
    projectType = selectedTemplate;
    console.log(
      `[getComprehensiveCodeAnalysis] Using selected template: ${projectType}`
    );
  }
    ...
    try {
      ...
      if (fileContent) {
        ...
        analysis.summary.fileTypes[fileName.split(".").pop() || "directory"] =
          (analysis.summary.fileTypes[
            fileName.split(".").pop() || "directory"
          ] || 0) + 1;

        console.log(
          `[getComprehensiveCodeAnalysis] Successfully analyzed: ${fileName} (${lines} lines)`
        );
      } else {
        console.log(
          `[getComprehensiveCodeAnalysis] Could not fetch content for: ${fileName}`
        );
      }
    } catch (error) {
      console.log(
        `[getComprehensiveCodeAnalysis] Error analyzing ${fileName}: ${error.message}`
      );
    }
  }
...
const calculateCodeQualityScore = (codeAnalysis) => {
  ...
  console.log(
    `   📊 Project Structure Subtotal: ${structureScore.points}/30 points`
  );
  ...
  console.log(
    `   📊 Security & Performance Subtotal: ${securityScore.points}/25 points`
  );
  ...
  console.log(
    `   📊 Modern Practices Subtotal: ${modernScore.points}/20 points`
  );
...
const evaluateProjectStructure = (codeAnalysis) => {
  ...
  Object.entries(criteria).forEach(([category, expectedFiles]) => {
    ...
    if (foundFiles === expectedFiles.length) {
      ...
      feedback.push(
        `⚠️ Partial ${category} structure (${foundFiles}/${expectedFiles.length})`
      );
    } else {
      feedback.push(
        `❌ Incomplete ${category} structure (${foundFiles}/${expectedFiles.length})`
      );
    }
    ...
    console.log(
      `   ${
        foundFiles > expectedFiles.length * 0.5 ? "✅" : "❌"
      } ${category}: ${foundFiles}/${
        expectedFiles.length
      } files (${categoryPoints} points)`
    );
  });
  ...
  mainFiles.forEach((file) => {
    if (codeAnalysis.files[file]) {
      ...
      if (commentRatio > 0.1) {
        ...
        details.push(
          `✅ High comment ratio: ${Math.round(commentRatio * 100)}%`
        );
      } else if (commentRatio > 0.05) {
        points += 3;
        feedback.push("✅ Good code documentation");
        details.push(
          `✅ Good comment ratio: ${Math.round(commentRatio * 100)}%`
        );
      } else {
        details.push(
          `❌ Low comment ratio: ${Math.round(commentRatio * 100)}%`
        );
      }
...
const evaluateSecurityAndPerformance = (codeAnalysis) => {
  ...
  if (codeAnalysis.files["package.json"]) {
    ...
    if (foundSecurityPackages.length >= 2) {
      ...
      details.push(
        `✅ Found security packages: ${foundSecurityPackages.join(", ")}`
      );
    } else {
      details.push(
        `❌ Limited security packages: ${
          foundSecurityPackages.join(", ") || "none"
        }`
      );
    }
    ...
    const foundPerformancePackages = performancePackages.filter((pkg) =>
      pkgContent.includes(pkg)
    );
    ...
    if (foundPerformancePackages.length >= 1) {
      ...
      details.push(
        `✅ Found performance packages: ${foundPerformancePackages.join(", ")}`
      );
    } else {
...
const evaluateModernPractices = (codeAnalysis) => {
  ...
  const hasContainerization = containerFiles.some((file) =>
    Object.keys(codeAnalysis.files).some((f) =>
      f.toLowerCase().includes(file.toLowerCase())
    )
  );
...
const calculateFunctionalityScore = (codeAnalysis) => {
  ...
  if (codeAnalysis.files["index.js"]) {
    ...
    if (
      indexContent.includes("mongoose") ||
      indexContent.includes("connect") ||
      indexContent.includes("database")
    ) {
      score += 10;
    ...
    if (
      indexContent.includes("auth") ||
      indexContent.includes("jwt") ||
      indexContent.includes("passport")
    ) {
      score += 10;
  ...
  console.log(
    `   📊 Core Functionality Subtotal: ${Math.min(30, score)}/30 points`
  );

  // 2. API Endpoints (25 points)
  console.log("\n🌐 2. API ENDPOINTS (25 points max):");
  const routeFiles = Object.keys(codeAnalysis.files).filter((file) =>
    file.includes("route")
  );
  ...
  console.log(
    `   📊 API Endpoints Subtotal: ${Math.min(25, score - 30)}/25 points`
  );
  ...
  console.log(
    `   📊 Data Models Subtotal: ${Math.min(25, score - 55)}/25 points`
  );
  ...
  console.log(
    `   📊 Configuration Subtotal: ${Math.min(20, score - 80)}/20 points`
  );
...
const calculateDocumentationScore = (codeAnalysis) => {
  ...
  if (codeAnalysis.files["README.md"] || codeAnalysis.files["README"]) {
    ...
    const readmeContent =
      codeAnalysis.files["README.md"]?.content ||
      codeAnalysis.files["README"]?.content ||
      "";
...
const calculateInnovationScore = (codeAnalysis) => {
  ...
  if (
    codeAnalysis.files["package.json"]?.content.includes("test") ||
...
async function getContributorsData(owner, repo) {
  ...
  const headers = {
    ...
    Authorization: `Bearer ${process.env.GITHUB_TOKEN}`, // Optional but helps with rate limits
  };
  ...
  for (const contributor of contributors) {
    const eventsUrl = `https://api.github.com/users/${contributor.login}/events/public`;
    ...
    for (const event of events) {
      if (event.type === "PushEvent") {
        ...
        break; // Only need first PushEvent with email
      }
    }
  }
...
async function checkRepoOwnership(userEmail, contributors) {
  if (!Array.isArray(contributors)) {
    contributors = [];
  }
  const ownershipValidated = contributors.some(
    (contributor) => contributor.email == userEmail
  );
