...
/** Non-empty body required by Mongoose; never shown to clients — `formatMessage` clears when `isDeletedForEveryone`. */
...
const recomputeConversationLastMessage = async (conversationId) => {
  ...
  const newest = await TeamMessage.findOne(visibleQuery).sort({ createdAt: -1 }).lean();
...
const getConversationMessages = async (
  ...
  const result = await TeamMessage.getConversationMessages(
    conversationId,
    currentUser._id,
    since ? { ...options, since } : options,
  );
...
const sendMessage = async (
  ...
  if (text.trim() === TEAM_MESSAGE_BODY_TOMBSTONE) {
    throw createHttpError("This message text is reserved by the system.", 400);
  }
  ...
  if (!contactCheck.blocked) {
    ...
    await TeamConversation.updateOne(
      ...
      { $pull: { hiddenForParticipants: receiverId } },
    );
  } else {
    await conversation.updateLastMessage({
...
/**
 * Delete a team message (WhatsApp-like).
 * scope=me: hide for current user only (any participant).
 * scope=everyone: hard-hide for both participants (sender only).
 */
const deleteMessage = async (currentUser, messageId, scopeRaw, auth = null) => {
  ...
  const scope =
    String(scopeFirst || "me").toLowerCase() === "everyone" ? "everyone" : "me";
  ...
  if (scope === "everyone") {
    if (message.senderId.toString() !== currentUser._id.toString()) {
      throw createHttpError("Only the sender can delete this message for everyone", 403);
    }
    if (message.isDeletedForEveryone) {
      throw createHttpError("Message already deleted for everyone", 400);
    }

    const convId = message.conversationId.toString();
    // Native collection update bypasses Mongoose document validation / middleware that can
    // still surface "text required" on some versions when using Model.updateOne + $set.
    const updated = await TeamMessage.collection.updateOne(
      { _id: new mongoose.Types.ObjectId(String(message._id)) },
      {
        $set: {
          ...
          deletedAt: new Date(),
          deletedForEveryoneBy: new mongoose.Types.ObjectId(String(currentUser._id)),
          text: TEAM_MESSAGE_BODY_TOMBSTONE,
        },
      },
    );
    ...
    return {
      ...
      message: formatMessage(fresh),
    };
  }

  if (!message.belongsToUser(currentUser._id)) {
    throw createHttpError("Unauthorized: You can only delete messages you are part of", 403);
  }
