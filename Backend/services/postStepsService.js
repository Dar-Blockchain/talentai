...
async function createPostStep(postStepData) {
  try {
    if (Array.isArray(postStepData)) {
      ...
      for (const stepData of postStepData) {
        ...
        if (stepData.data?.config) {
          if (!stepData.data.config.nodeNumber) {
            const nextNodeResult = await getNextNodeNumber(stepData.postId);
      ...
      if (savedPostSteps.length > 0) {
        ...
        await updatePostWithSteps(postId, savedPostSteps.map(step => step._id));
      }
      ...
      if (postStepData.data?.config) {
        if (!postStepData.data.config.nodeNumber) {
          const nextNodeResult = await getNextNodeNumber(postStepData.postId);
          if (nextNodeResult.success) {
      ...
      await updatePostWithSteps(savedPostStep.postId, [savedPostStep._id]);
...
async function updatePostWithSteps(postId, stepIds) {
  try {
...
async function removeStepFromPost(postId, stepId) {
  try {
    const post = await Post.findById(postId);
    if (!post) {
      throw new Error('Post non trouvé');
...
async function getAllPostSteps() {
  try {
...
async function getPostStepById(id) {
  try {
...
async function getPostStepByNodeId(nodeId) {
  try {
...
async function getPostStepsByPostId(postId) {
  try {
...
async function updatePostStep(id, updateData) {
  try {
...
async function updatePostStepByNodeId(nodeId, updateData) {
  try {
...
async function deletePostStep(id) {
  try {
    ...
    await removeStepFromPost(deletedPostStep.postId, deletedPostStep._id);
...
async function deletePostStepByNodeId(nodeId) {
  try {
...
async function getPostStepsByType(type) {
  try {
...
async function getPostStepsByNodeType(nodeType) {
  try {
...
async function updateNodeConfig(nodeId, configData) {
  try {
    const updatedPostStep = await Post_Steps.findOneAndUpdate(
      { id: nodeId },
      {
...
async function updateNodePosition(nodeId, positionData) {
  try {
    const updatedPostStep = await Post_Steps.findOneAndUpdate(
      { id: nodeId },
      {
...
async function getNextNodeNumber(postId) {
  try {
...
async function createNode(postId, nodeData) {
  try {
    const nextNodeResult = await getNextNodeNumber(postId);
    if (!nextNodeResult.success) {
    ...
    await updatePostWithSteps(postId, [savedPostStep._id]);
...
async function saveMultipleNodes(postId, nodesData) {
  try {
    ...
    await updatePostWithSteps(postId, savedNodes.map(node => node._id));
...
async function getNodesBySpecificType(postId, nodeType) {
  try {
...
async function addStepsToPost(postId, stepsData) {
  try {
    ...
    for (const step of stepsWithPostId) {
      try {
        const existingStep = await getPostStepByNodeId(step.id);

        if (existingStep.success && existingStep.data) {
          const updateResult = await updatePostStepByNodeId(step.id, step);
          ...
          const createResult = await createPostStep([step]);
...
module.exports.updatePostWithSteps = updatePostWithSteps;
module.exports.removeStepFromPost = removeStepFromPost;
...
module.exports.getPostStepsByNodeType = getPostStepsByNodeType;
