const { withTimeout, safeEmit, AI_TIMEOUT_MS, resetInterTurnPauseTimer, startIntelligentSilenceMonitoring } = require('../interviewUtils');
const { handleAIDecision } = require('./decision.handler');
const logger = require('../../../utils/logger');

async function handleSkipQuestion(socket, { service, processing }) {
  ...
  if (processing.has(sessionId)) {
    logger.warn('Skip ignored — AI call in progress', { sessionId });
    return;
  }
