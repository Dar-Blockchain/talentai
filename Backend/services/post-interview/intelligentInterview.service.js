 ...
 *   interviewAnalysis.js     — pure analysis helpers (no LLM for pure fns, LLM for analysis)
 *   interviewTermination.js  — termination logic
 *   interviewSetup.js        — persona, greeting, report generation
 */
'use strict';
...
} = require('./interviewAnalysis');

const { updateQualityCounters, shouldEndInterview } = require('./interviewTermination');
...
class IntelligentInterviewService {
  ...
  async startInterview(sessionId, userConfig, candidateId, onGreetingChunk = null) {
    try {
      ...
      try {
        ...
        if (jobId) {
          ...
          if (post?.jobDetails) {
            ...
            console.log(`✅ [Service] Full JD loaded: ${jobDescription.title} at ${companyName}`);
      ...
      // Build agent persona (ONE LLM call — sets evaluation framework + ideal candidate)
      ...
      if (jobDescription) {
        try {
          ...
          console.log(`✅ [Service] Persona-driven coverage initialized: ${Object.keys(frameworkAreas).length} areas`);
          ...
          console.log(`📋 [Service] JD skills checklist initialized: ${jdSkillsChecklist.length} skills`);
        } catch (personaError) {
      ...
      console.log(`✅ [Service] Interview timing: target ${totalMinutes}min, max ${maxDurationMinutes}min, ${coverageAreas.length} areas`);
      ...
      console.log(`✅ [Service] Interview ${sessionId} started successfully`);
      ...
      console.error('❌ [Service] CRITICAL: Failed to start interview:', error.message);
      throw error;
    }
  }
  ...
  async processCandidateResponse(sessionId, transcript, audioMetadata = {}) {
    try {
      ...
      // ── STEP 3: Apply coverage updates ──
      let finalCoverage = { ...session.coverage };
      if (analysis.coverage?.areasImpacted?.length > 0) {
        console.log(`📊 [Coverage] LLM areasImpacted: ${analysis.coverage.areasImpacted.map(a => `${a.area}(+${a.increase})`).join(', ')}`);
        for (const impact of analysis.coverage.areasImpacted) {
          ...
          if (!finalCoverage.areas[matchedAreaKey]) {
            ...
            if (matchedAreaKey) console.log(`🔄 [Coverage] Fuzzy matched "${impact.area}" → "${matchedAreaKey}"`);
          }
          ...
          if (matchedAreaKey && finalCoverage.areas[matchedAreaKey]) {
            ...
            if (isPassSkip && increase === 0) {
              console.log(`⭐ [Coverage] Pass/skip detected for "${impact.area}" — gap recorded, no coverage credit`);
            ...
            console.warn(`⚠️ [Coverage] Area mismatch: LLM returned "${impact.area}" but session has [${Object.keys(finalCoverage.areas).join(', ')}]`);
          }
        }
        ...
        // No areasImpacted — still record pass/skip gap
        ...
        if (isPassSkipNoAreas && targetArea && finalCoverage.areas[targetArea]) {
          ...
          console.log(`⭐ [Coverage] Pass/skip (no areas from LLM) for "${targetArea}" — gap recorded, no coverage credit`);
      ...
      // ── STEP 4: Quality filter + termination check ──
      const qualityScore = analysis.quality?.score || 0;
      ...
      if (isLowQuality) console.log(`🚫 [Pipeline] Low quality (${qualityScore}) — generating from gaps only`);

      // ── STEP 5: Decide question strategy ──
      ...
      // ── STEP 5.5: Select question style ──
      ...
      // ── STEP 5.7: Retrieve RAG context (~100ms) ──
      let ragContext = '';
      try {
        ...
        if (jobId) {
          ...
          if (ragContext) console.log(`📚 [Step 5.7] RAG context retrieved: ${ragContext.length} chars`);
        }
      } catch (ragErr) {
  ...
  /**
   * Handle silence detection — only acts on extended silence (20s+) to move to next question.
   */
  async handleSilence(sessionId, silenceDuration) {
    try {
      ...
      console.log(`⭐  [Silence] Extended silence (${silenceDuration}s) - generating next question`);
