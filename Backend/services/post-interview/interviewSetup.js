...
const { buildGreetingSystem, buildGreetingUser, buildSilenceUser } = require('./prompts/generationPrompts');
const { buildAgentPersonaUser, buildFinalReportUser } = require('./prompts/reportPrompts');
...
async function generateIntelligentGreeting(config, onChunk = null, persona = null) {
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      lastError = error;
      console.error(`❌ [Greeting] Attempt ${attempt}/${maxRetries} failed:`, error.message);
...
async function generateSilencePrompt(session, silenceData) {
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      lastError = error;
      console.error(`❌ [Silence] Attempt ${attempt}/${maxRetries} failed:`, error.message);
      if (attempt < maxRetries) {
        await new Promise(resolve => setTimeout(resolve, 500));
      }
    }
